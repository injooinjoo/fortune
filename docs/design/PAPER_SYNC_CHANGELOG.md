...
| 2026-04-04 | `KAN-224` | 메시지/인사이트 목록의 보조 텍스트 가독성 보정 | `CharacterListPanel`, `FortuneRecordCard` | No artboard or route inventory changed. Tightened the secondary metadata styles on message lists and insight history cards so timestamps, previews, and supporting copy stay readable without changing the Paper route inventory or component contract. |
| 2026-04-04 | `KAN-224` | CI formatter 출력과 readability 패치 재정렬 | `CharacterListPanel`, `FortuneRecordCard` | No artboard or route inventory changed. Re-applied the readability patch in the exact repository-side formatter shape expected by Flutter CI/CD so the Paper guard and format gate both see the same governed UI delta on the latest SHA. |
