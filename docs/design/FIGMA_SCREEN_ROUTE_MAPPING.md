...
| `chat__onboarding__character_intro` | Character Chat Onboarding | `internal_state` | `#/chat` | `SwipeHomeShell` first-run gate rendering `SignupScreen` or `OnboardingPage` before the chat shell | `lib/features/character/presentation/pages/swipe_home_shell.dart`, `lib/screens/auth/signup_screen.dart`, `lib/screens/onboarding/onboarding_page.dart`, `lib/services/storage_service.dart` | `live` | Unified soft-gate plus authenticated profile-completion flow inside `/chat`; legacy `CharacterOnboardingPage` no longer exists |
