...
- 현재 카탈로그 범위: active route surface `18`개, live `14`, placeholder `4`
- canonical governance roots: `89:2`-`94:2`
- latest chat refresh append: `95:2` (`20 Chat Home / Character`)
...
실제 수동 작업은 [FIGMA_APPEND_RENAME_RUNBOOK.md](./FIGMA_APPEND_RENAME_RUNBOOK.md)의 current canonical range `89:2`-`94:2` 기준으로 진행하되, `KAN-86`에서 append된 채팅 새로고침 페이지 `95:2`는 병합 전 별도 검수 대상으로 확인한다.
