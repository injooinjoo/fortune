...
- latest chat refresh append: `180:2` (`20 Chat Home / Character`)
...
실제 수동 작업은 [FIGMA_APPEND_RENAME_RUNBOOK.md](./FIGMA_APPEND_RENAME_RUNBOOK.md)의 current canonical range `89:2`-`94:2` 기준으로 진행하되, `KAN-124`에서 append된 premium 채팅 새로고침 페이지 `180:2`를 최신 검수 대상으로 확인한다. 기존 `95:2`는 historical append로 보존한다.
