...
class FortuneLoveWidget : AppWidgetProvider() {
    ...
    companion object {
        internal fun updateAppWidget(
            ...
            val widgetDataJson = prefs.getString("widget_fortune_love", null)
            
            // Parse widget data
            var compatibilityScore = "?"
            var partnerName = "상대방"
            var loveMessage = "연애운을 확인하려면 앱을 열어주세요"
            var lastUpdated = ""
            
            if (widgetDataJson != null) {
                try {
                    val data = JSONObject(widgetDataJson)
                    compatibilityScore = data.optString("compatibilityScore", "?")
                    partnerName = data.optString("partnerName", partnerName)
                    loveMessage = data.optString("message", loveMessage)
            ...
            val intent = Intent(context, MainActivity::class.java).apply {
                ...
                putExtra("widget_type", "love_fortune")
            }
