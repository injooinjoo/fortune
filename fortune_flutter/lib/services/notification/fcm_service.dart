...
class NotificationSettings {
  ...
  factory NotificationSettings.fromJson(Map<String, dynamic> json) {
    return NotificationSettings(
      ...
      dailyFortune: json['dailyFortune'] ?? true,
      tokenAlert: json['tokenAlert'] ?? true,
...
class FCMService {
  ...
  Future<void> _requestPermission() async {
    ...
    Logger.info('알림 권한 상태: ${settings.authorizationStatus}');
  ...
  Future<void> _getToken() async {
    try {
      ...
      _fcm.onTokenRefresh.listen((newToken) async {
        Logger.info('FCM 토큰 갱신: $newToken');
        _fcmToken = newToken;
  ...
  void _setupMessageListeners() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      Logger.info('포그라운드 메시지 수신: ${message.messageId}');
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      Logger.info('알림 탭으로 앱 열림: ${message.messageId}');
  ...
  Future<void> _checkInitialMessage() async {
    ...
    if (message != null) {
      Logger.info('초기 메시지로 앱 실행: ${message.messageId}');
      _handleNotificationTap(message.data);
    }
  }
  ...
  void _handleNotificationTap(Map<String, dynamic> data) {
    ...
    switch (type) {
      ...
      case 'promotion':
        // 프로모션 상세 페이지로 이동
        Logger.info('프로모션 페이지로 이동: $targetId');
        break;
      default:
        Logger.info('알 수 없는 알림 타입: $type');
    }
  }
