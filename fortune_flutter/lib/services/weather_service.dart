...
class WeatherData {
  ...
  factory WeatherData.fromJson(Map<String, dynamic> json) {
    return WeatherData(
      temperature: json['main']['temp'].toDouble() - 273.15, // Kelvin to Celsius
      ...
      fineDust: json['air_quality']?.toDouble() ?? 0.0,
  ...
  static Future<WeatherData> getWeatherData({
    ...
    if (_cache.containsKey(cacheKey)) {
      ...
      if (DateTime.now().difference(cached.timestamp) < _cacheExpiry) {
        Logger.info('Weather: Using cached data for $cacheKey');
        return cached;
      }
    }

    try {
      final url = Uri.parse('$_baseUrl/weather?lat=$latitude&lon=$longitude&appid=$_apiKey');
      ...
      if (response.statusCode == 200) {
        ...
        Logger.info('Weather: Fetched new data for $cacheKey');
