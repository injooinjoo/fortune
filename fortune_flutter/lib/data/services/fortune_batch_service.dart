...
enum BatchPackageType {
  ...
  dailyRefresh('daily_refresh', '일일 갱신 패키지', 2),
  loveSingle('love_single', '연애운 패키지 (솔로)', 4),
  loveCouple('love_couple', '연애운 패키지 (커플)', 5),
  career('career', '커리어 패키지', 6),
  luckyItems('lucky_items', '행운 아이템 패키지', 3),
...
class FortuneBatchService {
  ...
  Future<List<BatchFortuneResult>> generateBatchFortunesByPackage({
    ...
    debugPrint('Generating batch fortunes for package: ${packageType.key}');
    ...
    try {
      ...
      debugPrint('Batch fortune generation failed: $e');
      // 폴백: 캐시된 데이터 반환
  ...
  Future<List<BatchFortuneResult>> generateBatchFortunesByTypes({
    ...
    debugPrint('Generating batch fortunes for types: ${fortuneTypes.join(', ')}');
    ...
    try {
      ...
      debugPrint('Custom batch fortune generation failed: $e');
      // 폴백: 개별 운세 생성
  ...
  /// 배치 응답 처리
  List<BatchFortuneResult> _processBatchResponse(Map<String, dynamic> data, String userId) {
    ...
    for (final fortuneData in fortunes) {
      ...
      final fortune = Fortune(
        ...
        overallScore: fortuneInfo['overall_score']?.toInt(),
  ...
  List<String> _getFortuneTypesForPackage(BatchPackageType packageType) {
    switch (packageType) {
      ...
      case BatchPackageType.luckyItems:
        return ['lucky-color', 'lucky-number', 'lucky-items', 'lucky-food', 'lucky-outfit'];
  ...
  Future<List<BatchFortuneResult>> _generateIndividualFortunes(
    ...
    for (final type in fortuneTypes) {
      try {
        ...
        // 개별 API 호출
        final response = await _apiClient.get('/api/fortune/$type');
        ...
        debugPrint('Failed to generate individual fortune for $type: $e');
      }
    }
  ...
  Future<Map<String, dynamic>> generateSystemFortunes({
    ...
    debugPrint('Generating system fortunes for: $fortuneType');
    ...
    // 기존 API 폴백
    throw UnimplementedError('System fortune generation not available in traditional API');
  }
  ...
  double calculateTokenSavings(BatchPackageType packageType) {
    ...
    const individualCosts = {
      ...
      'hourly': 1, 'weekly': 2, 'monthly': 2, 'yearly': 3, 'love': 2,
      'marriage': 3, 'blind-date': 2, 'celebrity-match': 2, 'couple-match': 3,
      'chemistry': 3, 'career': 3, 'wealth': 3, 'business': 4, 'lucky-color': 1,
      ...
      'children': 3, 'parenting': 3, 'pregnancy': 3, 'family-harmony': 2,
    };
