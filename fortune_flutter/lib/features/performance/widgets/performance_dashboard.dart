      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16))
            _buildStatRow('Cache Hits', _cacheStats['hits']?.toString() ?? '0'),
            _buildStatRow('Cache Misses', _cacheStats['misses']?.toString() ?? '0'),
            _buildStatRow('Hit Rate', '${_cacheStats['hitRate'] ?? '0.0'}%'),
            _buildStatRow('Memory Cache Size', _cacheStats['memoryCacheSize']?.toString() ?? '0'),
            _buildStatRow('Disk Cache Keys', _cacheStats['diskCacheKeys']?.toString() ?? '0'),
            const SizedBox(height: 16))
            ...
            Column(
              ...
              children: [
                ...
                LinearProgressIndicator(
                  value: double.tryParse(_cacheStats['hitRate']?.toString() ?? '0') ?? 0 / 100,
                  backgroundColor: Colors.grey.withValues(alpha: 0.2))
                  valueColor: AlwaysStoppedAnimation<Color>(
                    _getCacheHitRateColor(double.tryParse(_cacheStats['hitRate']?.toString() ?? '0') ?? 0),
                  ))
                ))
              ])
            ),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _buildMetricChip('Count', metric['count'].toString()),
                      ])
                    ),
                  ])
                ),
            ...
            SizedBox(
              ...
              child: BarChart(
                BarChartData(
                  ...
                  maxY: (apiMetrics['p95'] as int).toDouble() * 1.2,
                  ...
                  barGroups: [
                    BarChartGroupData(
                      ...
                      barRods: [
                        BarChartRodData(
                          toY: (apiMetrics['avg'] as int).toDouble(),
                          color: Theme.of(context).colorScheme.primary)
                          width: 40)
                    ...
                    BarChartGroupData(
                      ...
                      barRods: [
                        BarChartRodData(
                          toY: (apiMetrics['p50'] as int).toDouble(),
                          color: Theme.of(context).colorScheme.secondary)
                          width: 40)
                          borderRadius: BorderRadius.circular(4))
                        ))
                    ...
                    BarChartGroupData(
                      ...
                      barRods: [
                        BarChartRodData(
                          toY: (apiMetrics['p95'] as int).toDouble(),
                          color: Theme.of(context).colorScheme.tertiary)
                          width: 40)
                          borderRadius: BorderRadius.circular(4))
                        ))
