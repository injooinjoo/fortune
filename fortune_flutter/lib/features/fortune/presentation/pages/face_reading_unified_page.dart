...
class _FaceReadingUnifiedPageState extends ConsumerState<FaceReadingUnifiedPage> {
  ...
  Future<void> _analyzeFaceReading() async {
    ...
    try {
      final inputData = {
        if (_selectedImage != null) 'hasPhoto': true,
        if (_selectedFaceShape != null) 'faceShape': _selectedFaceShape,
        if (_selectedEyeType != null) 'eyeType': _selectedEyeType,
        if (_selectedOverallImpression != null) 'impression': _selectedOverallImpression,
      };
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('관상 분석 중 오류가 발생했습니다: $e')),
        );
      }
    }
  }
