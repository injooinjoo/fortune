...
enum InvestmentSector {
  stocks('주식', '국내/해외 주식', Icons.trending_up_rounded, [Color(0xFF2563EB), Color(0xFF1D4ED8)]),
  realestate('부동산', '아파트, 오피스텔, 토지', Icons.home_rounded, [Color(0xFF059669), Color(0xFF047857)]),
  crypto('암호화폐', '비트코인, 알트코인', Icons.currency_bitcoin_rounded, [Color(0xFFFF6F00), Color(0xFFE65100)]),
  auction('경매', '부동산/물품 경매', Icons.gavel_rounded, [Color(0xFF7C3AED), Color(0xFF6D28D9)]),
  lottery('로또', '로또 번호 추천', Icons.confirmation_number_rounded, [Color(0xFFFFB300), Color(0xFFF57C00)]),
  funds('펀드/ETF', '인덱스, 섹터별 펀드', Icons.account_balance_rounded, [Color(0xFF3B82F6), Color(0xFF2563EB)]),
  gold('금/원자재', '금, 은, 원유', Icons.diamond_rounded, [Color(0xFFFACC15), Color(0xFFEAB308)]),
  bonds('채권', '국채, 회사채', Icons.receipt_long_rounded, [Color(0xFF10B981), Color(0xFF059669)]),
  startup('스타트업', '크라우드펀딩', Icons.rocket_launch_rounded, [Color(0xFFEC4899), Color(0xFFDB2777)]),
...
class _InvestmentFortuneEnhancedPageState extends ConsumerState<InvestmentFortuneEnhancedPage> 
  ...
  Widget _buildGoalSelector(InvestmentFortuneData data) {
    final options = [
      ...
      {'value': 'speculation', 'label': '단기 수익', 'icon': Icons.bolt_rounded},
      {'value': 'retirement', 'label': '노후 준비', 'icon': Icons.elderly_rounded},
    ];
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryCard('투자 프로필', [
            '위험 성향: ${_getRiskToleranceLabel(data.riskTolerance)}',
            '투자 경험: ${_getExperienceLabel(data.investmentExperience)}',
            '투자 목표: ${_getGoalLabel(data.investmentGoal)}',
            '투자 기간: ${_getHorizonLabel(data.investmentHorizon)}',
          ]),
          const SizedBox(height: 16),
