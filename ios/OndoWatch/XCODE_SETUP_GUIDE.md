...
```
OndoWatch/
├── OndoWatchApp.swift        ← 교체
├── ContentView.swift            ← 교체
├── Info.plist                   ← 교체
├── OndoWatch.entitlements    ← 추가
├── Models/
...
1. OndoWatch Target 선택
2. **Signing & Capabilities** 탭 클릭
3. **+ Capability** 클릭
4. **App Groups** 추가
5. `group.com.beyond.ondo` 선택 (Runner와 동일)

### Step 2: Entitlements 확인
`OndoWatch.entitlements` 파일이 제대로 연결되었는지 확인:
- Build Settings > Code Signing Entitlements: `OndoWatch/OndoWatch.entitlements`
...
### Bundle Identifier
- OndoWatch: `com.beyond.ondo.watchapp`
- OndoWatch Complication: `com.beyond.ondo.watchapp.complication`
...
3. 설정:
   - **Product Name**: `OndoWatchComplication`
...
- [ ] OndoWatch Target 생성됨
- [ ] App Groups 설정 (`group.com.beyond.ondo`)
