...
serve(async (req) => {
  ...
  try {
    ...
    if (cachedResult?.result) {
      ...
      const prompt = isCsat ? `당신은 수능 전담 컨설턴트이자 입시 전략가입니다.
수험생의 컨디션과 실전 전략을 구체적으로 제시하며, 수능 당일 루틴과 과목별 집중 포인트를 제공합니다.
뻔한 위로 대신, 실제 점수를 끌어올리는 실전 디테일을 중심으로 조언하세요.
...
11. ${ddayLabel}에 맞는 시기적절한 조언` : `당신은 20년 경력의 시험운 전문 상담가입니다.
