...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🎯 [FamilyRelationship] Cohort: ${JSON.stringify(cohortData)}`)

    const poolResult = await getFromCohortPool(supabaseClient, 'family-relationship', cohortHash)

    if (poolResult) {
      console.log('✅ [FamilyRelationship] Cohort Pool 히트! LLM 호출 생략')
...
${special_question ? '특별 질문에 대한 답변도 specialAnswer에 포함해주세요.' : ''}`
    ...
    saveToCohortPool(supabaseClient, 'family-relationship', cohortHash, cohortData, result)
      .catch(e => console.error('[FamilyRelationship] Cohort 저장 오류:', e))
