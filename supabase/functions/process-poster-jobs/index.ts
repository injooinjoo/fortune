...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Push 발송
    await sendCompletionPush(admin, job);
...
async function insertResultCardMessage(
  ...
  // EmbeddedResultPayload — RN 측 chat-shell.ts 의 ChatShellEmbeddedResultMessage 와 호환.
  const message = {
    id: messageId,
    ...
    title: posterTypeLabel(job.poster_type),
    payload: {
      kind: job.poster_type,
      fortuneType: job.poster_type,
      resultKind: job.poster_type,
      // poster-guide.tsx 가 rawApiResponse.data.imageUrl 을 읽음.
      // 동일 shape 으로 INSERT 하기 위해 rawApiResponse 미러링.
      imageUrl,
      generatedAt,
      rawApiResponse: {
        success: true,
...
/**
 * 완료 push 발송 — 기존 sendCharacterDmPush 재사용.
 * type: 'character_dm' 으로 보내면 클라이언트가 채팅 화면으로 라우팅.
 */
async function sendCompletionPush(
  ...
  await sendCharacterDmPush({
    ...
    messageText: `${label} 분석 결과 도착! 확인해봐 👀`,
    messageId: `result-${job.id}`,
    type: 'character_dm',
    scheduledId: job.id,
  });
}
