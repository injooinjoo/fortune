...
- 반드시 100자 이내의 핵심 조언만 (100자 초과 금지)
- 인사말/서두 없이 바로 본론
- 구체적이고 실용적인 내용
- 한국어

${idiom}의 의미를 녹여 오늘 하루 핵심 조언을 100자 이내로 압축.`;
        } else {
          prompt = `오늘의 ${categoryName} 조언을 작성하세요.
...
점수에 맞는 톤으로 ${categoryName} 핵심 조언을 100자 이내로 작성.`;
        }
        ...
        const response = await llm.generate([
          {
            role: 'system',
            content: '운세 전문가. 인사말/서두 없이 요점만 간결하게 전달. "안녕하세요", "~입니다" 같은 인사 금지.'
          },
          {
            role: 'user',
            content: prompt
