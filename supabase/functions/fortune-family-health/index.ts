  ...
  try {
    ...
    if (poolResult) {
      ...
      const blurredSections = isBlurred
        ? ['healthCategories', 'dietAdvice', 'exerciseGuide', 'monthlyFlow', 'recommendations', 'warnings', 'specialAnswer']
        : []
...
${special_question ? '특별 질문에 대한 답변도 specialAnswer에 포함해주세요.' : ''}`
    ...
    const isBlurred = !isPremium
    const blurredSections = isBlurred
      ? ['healthCategories', 'seasonalAdvice', 'familySynergy', 'monthlyFlow', 'familyAdvice', 'recommendations', 'warnings', 'specialAnswer']
      : []
