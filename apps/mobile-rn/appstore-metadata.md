# App Store Metadata - 온도 (Ondo)

## Basic Info
- **App Name**: 온도
- **Subtitle**: AI 운세 & 스토리 채팅
- **Bundle ID**: com.beyond.fortune
- **SKU**: ondo-fortune-2026
- **Primary Language**: Korean

## Category
- **Primary**: Entertainment
- **Secondary**: Lifestyle

## URLs
- **Privacy Policy**: https://hayjukwfcsdmppairazc.supabase.co/functions/v1/legal-pages/privacy-policy
- **Terms of Service**: https://hayjukwfcsdmppairazc.supabase.co/functions/v1/legal-pages/terms-of-service
- **Support URL**: https://hayjukwfcsdmppairazc.supabase.co/functions/v1/legal-pages
- **Support Email**: injoo1222@naver.com

## Description (Korean)

온도는 AI가 만들어가는 당신만의 운세와 스토리 세계입니다.

사주, 타로, 별자리, MBTI, 궁합부터 꿈 해몽, 관상, 전생 리딩까지 - 30가지 이상의 운세를 전문 AI 캐릭터와의 대화로 경험하세요.

[주요 기능]
- AI 운세 상담: 오늘 운세, 사주, 타로, 별자리, 궁합 등 다양한 운세를 채팅으로 간편하게
- 스토리 채팅: 매력적인 AI 캐릭터들과 이어가는 인터랙티브 스토리
- 맞춤 분석: 생년월일, MBTI 등 프로필 기반 개인화된 인사이트
- 30+ 운세 카테고리: 연애운, 재물운, 커리어운, 건강운, 포춘쿠키, OOTD 코디 등
- 다양한 전문가 캐릭터: 하늘, 현우, 스텔라, 로제 등 각 분야 전문 AI 상담사

매일 새로운 운세와 인사이트로 하루를 시작하세요.
오늘의 행운 포인트, 추천 액션, 주의 포인트까지 한눈에 확인할 수 있습니다.

* 본 앱의 모든 운세 콘텐츠는 AI가 생성한 오락 목적의 콘텐츠이며, 전문적인 조언을 대체하지 않습니다.

## Keywords (100 chars max)
운세,사주,타로,별자리,MBTI,궁합,AI,채팅,꿈해몽,관상,연애운,재물운

## Age Rating
- **Rating**: 12+ (Infrequent/Mild Simulated Gambling - fortune telling content)
- Contests: None
- Gambling: None
- Horror/Fear: None
- Mature/Suggestive: Infrequent/Mild (romance story content)
- Medical/Treatment: None
- Profanity: None
- Violence: None

## App Review Notes
This is an AI-powered fortune telling and interactive story app. All fortune content is AI-generated entertainment and is clearly labeled as such. The app includes a disclaimer visible to users.

No login required. Users land directly on the main chat/fortune screen upon launch. All fortune features (30+ categories) are freely accessible without creating an account. Login is available optionally via the profile screen for saving history and syncing across devices.

## App Review Info (App Store Connect)
- Sign-in required: NO (uncheck the box)
- Demo account: Not needed
- Review notes: "No login required to access fortune features. Users land directly on the main screen."

## Screenshots (6.7" iPhone 16 Pro Max, 1320x2868)
1. 16promax-chat-home.png - Main message/character list
2. 16promax-character-chat.png - AI character conversation
3. 16promax-fortune-tab.png - Fortune counselor categories
4. 16promax-profile.png - User profile & settings
5. 16promax-fortune-chat.png - Fortune result (daily fortune)
