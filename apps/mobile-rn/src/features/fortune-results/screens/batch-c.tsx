...
function PersonalityDnaResult(props: FortuneResultComponentProps) {
  ...
  // --- 1. DNA Code Hero ---
  const dnaCode = str(raw.dnaCode ?? raw.dna_code, '');
  const dnaEmoji = str(raw.emoji, '🧬');
  const dnaTitle = str(raw.title, '');
  const rarityLevel = str(raw.rarityLevel ?? raw.rarity_level, '');
  ...
  const rarityColorMap: Record<string, string> = {
    ...
    rare: '#3B82F6',
    uncommon: '#22C55E',
    common: fortuneTheme.colors.textTertiary,
  };
  ...
  const rarityColor = rarityColorMap[rarityLevel] ?? fortuneTheme.colors.textTertiary;
  const rarityLabel = rarityLabelMap[rarityLevel] ?? '';

  // --- 2. Traits ---
  const traits = strArr(raw.traits);

  // --- 3. Love Style ---
  const loveStyle = obj(raw.loveStyle ?? raw.love_style);
  const loveTitle = str(loveStyle.title);
  const loveDesc = str(loveStyle.description);
  const whenDating = str(loveStyle.when_dating ?? loveStyle.whenDating);
  const afterBreakup = str(loveStyle.after_breakup ?? loveStyle.afterBreakup);

  // --- 4. Work Style ---
  const workStyle = obj(raw.workStyle ?? raw.work_style);
  const workTitle = str(workStyle.title);
  const asBoss = str(workStyle.as_boss ?? workStyle.asBoss);
  const atCompanyDinner = str(workStyle.at_company_dinner ?? workStyle.atCompanyDinner);
  const workHabit = str(workStyle.work_habit ?? workStyle.workHabit);

  // --- 5. Daily Matching ---
  const dailyMatching = obj(raw.dailyMatching ?? raw.daily_matching);
  const cafeMenu = str(dailyMatching.cafe_menu ?? dailyMatching.cafeMenu);
  const netflixGenre = str(dailyMatching.netflix_genre ?? dailyMatching.netflixGenre);
  const weekendActivity = str(dailyMatching.weekend_activity ?? dailyMatching.weekendActivity);

  // --- 6. Compatibility ---
  const compatibility = obj(raw.compatibility);
  const compatFriend = obj(compatibility.friend);
  const compatLover = obj(compatibility.lover);
  const compatColleague = obj(compatibility.colleague);

  // --- 7. Fun Stats ---
  const funStats = obj(raw.funStats ?? raw.fun_stats);
  const rarityRank = str(funStats.rarity_rank ?? funStats.rarityRank);
  const celebrityMatch = str(funStats.celebrity_match ?? funStats.celebrityMatch);
  const percentageInKorea = str(funStats.percentage_in_korea ?? funStats.percentageInKorea);

  // --- 8. Daily Fortune ---
  const dailyFortune = obj(raw.dailyFortune ?? raw.daily_fortune);
  const luckyColor = str(dailyFortune.luckyColor ?? dailyFortune.lucky_color);
  const luckyNumber = num(dailyFortune.luckyNumber ?? dailyFortune.lucky_number, 0);
  const energyLevel = str(dailyFortune.energyLevel ?? dailyFortune.energy_level);
  const recommendedActivity = str(dailyFortune.recommendedActivity ?? dailyFortune.recommended_activity);
  const caution = str(dailyFortune.caution);

  // --- Fallbacks from useResultData ---
  const summary =
    result.summary ||
    '성향의 결이 더 선명해지는 날이에요. 유형보다 축을 읽어야 길이 보입니다.';
  ...
  const spectrumItems =
    result.metrics.length >= 4
      ? result.metrics.slice(0, 4).map((m, i) => ({
          label: m.label,
          value: Number(m.value) || [78, 69, 81, 88][i] || 75,
          highlight: m.note || '',
        }))
      : [
          { label: '표현력', value: 78, highlight: '말은 짧아도 핵심은 또렷하게 전달됩니다.' },
          { label: '집중력', value: 69, highlight: '한 번 몰입하면 깊지만, 진입 준비가 필요합니다.' },
          { label: '회복력', value: 81, highlight: '흔들려도 다시 중심으로 돌아오는 속도가 빠릅니다.' },
          { label: '공감력', value: 88, highlight: '상대의 감정선을 빨리 읽는 편입니다.' },
        ];

  const specialTip =
    result.specialTip ||
    '너무 많은 가능성을 한 번에 붙잡기보다, 가장 납득되는 한 가지를 먼저 고르는 편이 운을 잘 타게 만듭니다.';
  ...
  return (
    <View style={{ gap: fortuneTheme.spacing.md }}>
      ...
      {hasRaw && dnaCode ? (
        ...
        >
          <AppText variant="displayLarge">{dnaEmoji}</AppText>
          <AppText variant="heading3">{dnaCode}</AppText>
          {dnaTitle ? (
            <AppText variant="bodyMedium" color={fortuneTheme.colors.textSecondary}>
              {dnaTitle}
            </AppText>
      ...
      {/* ============================================================ */}
      {traits.length > 0 ? (
        <SectionCard title="성격 특성" description="당신을 구성하는 핵심 키워드입니다.">
      ...
      {/* ============================================================ */}
      {loveTitle ? (
        <SectionCard title="연애 스타일" description={loveTitle}>
          <View style={{ gap: fortuneTheme.spacing.sm }}>
            {loveDesc ? (
              <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                {loveDesc}
            ...
            {whenDating ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <AppText variant="labelLarge">연애 중</AppText>
                <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                  {whenDating}
                </AppText>
              </Card>
            ...
            {afterBreakup ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <AppText variant="labelLarge">이별 후</AppText>
                <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                  {afterBreakup}
                </AppText>
              </Card>
      ...
      {/* ============================================================ */}
      {workTitle ? (
        <SectionCard title="업무 스타일" description={workTitle}>
          <View style={{ gap: fortuneTheme.spacing.sm }}>
            {asBoss ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <AppText variant="labelLarge">내가 팀장이라면</AppText>
                <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                  {asBoss}
                </AppText>
              </Card>
            ...
            {atCompanyDinner ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <AppText variant="labelLarge">회식 자리에서</AppText>
                <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                  {atCompanyDinner}
                </AppText>
              </Card>
            ...
            {workHabit ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <AppText variant="labelLarge">업무 습관</AppText>
                <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                  {workHabit}
                </AppText>
              </Card>
      ...
      {/* ============================================================ */}
      {/*  5. 일상 매칭 (dailyMatching)                                  */}
      {/* ============================================================ */}
      {(cafeMenu || netflixGenre || weekendActivity) ? (
        <SectionCard title="일상 매칭" description="당신의 성향이 일상에서 드러나는 방식이에요.">
          <MetricGrid
            items={[
              ...(cafeMenu ? [{ label: '카페 메뉴', value: cafeMenu, note: '' }] : []),
              ...(netflixGenre ? [{ label: '넷플릭스 장르', value: netflixGenre, note: '' }] : []),
              ...(weekendActivity ? [{ label: '주말 활동', value: weekendActivity, note: '' }] : []),
            ]}
          />
        </SectionCard>
      ) : null}
      ...
      {/*  6. 궁합 (compatibility)                                      */}
      {/* ============================================================ */}
      {(str(compatFriend.mbti) || str(compatLover.mbti) || str(compatColleague.mbti)) ? (
        <SectionCard title="궁합" description="관계 유형별 최적의 파트너입니다.">
          <View style={{ gap: fortuneTheme.spacing.sm }}>
            {str(compatFriend.mbti) ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <AppText variant="labelLarge">베스트 프렌드</AppText>
                  <AppText variant="labelMedium" color={fortuneTheme.colors.accentSecondary}>
                    {str(compatFriend.mbti)}
                  </AppText>
                </View>
                {str(compatFriend.description) ? (
                  <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                    {str(compatFriend.description)}
                  </AppText>
                ) : null}
            ...
            {str(compatLover.mbti) ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <AppText variant="labelLarge">이상적인 연인</AppText>
                  <AppText variant="labelMedium" color={fortuneTheme.colors.accentSecondary}>
                    {str(compatLover.mbti)}
                  </AppText>
                </View>
                {str(compatLover.description) ? (
                  <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                    {str(compatLover.description)}
                  </AppText>
                ) : null}
            ...
            {str(compatColleague.mbti) ? (
              <Card style={{ gap: fortuneTheme.spacing.xs }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <AppText variant="labelLarge">최고의 동료</AppText>
                  <AppText variant="labelMedium" color={fortuneTheme.colors.accentSecondary}>
                    {str(compatColleague.mbti)}
                  </AppText>
                </View>
                {str(compatColleague.description) ? (
                  <AppText variant="bodySmall" color={fortuneTheme.colors.textSecondary}>
                    {str(compatColleague.description)}
                  </AppText>
                ) : null}
      ...
      {/*  7. 재밌는 통계 (funStats)                                     */}
      {/* ============================================================ */}
      {(rarityRank || celebrityMatch || percentageInKorea) ? (
        <SectionCard title="재밌는 통계" description="당신의 성격 DNA는 얼마나 특별할까요?">
          <MetricGrid
            items={[
              ...(rarityRank ? [{ label: '희귀도 순위', value: rarityRank, note: '' }] : []),
              ...(celebrityMatch ? [{ label: '닮은 유명인', value: celebrityMatch, note: '' }] : []),
              ...(percentageInKorea ? [{ label: '한국 내 비율', value: percentageInKorea, note: '' }] : []),
            ]}
          />
        </SectionCard>
      ) : null}
        ...
        <SectionCard title="오늘의 흐름" description="오늘 하루를 위한 맞춤 가이드입니다.">
          <MetricGrid
            items={[
              ...
              ...(luckyNumber ? [{ label: '행운의 숫자', value: String(luckyNumber), note: '' }] : []),
              ...(energyLevel ? [{ label: '에너지 레벨', value: energyLevel, note: '' }] : []),
              ...(recommendedActivity ? [{ label: '추천 활동', value: recommendedActivity, note: '' }] : []),
            ]}
          />
          {caution ? (
            <InsetQuote text={caution} />
          ) : null}
        </SectionCard>
      ) : null}

      {/* ============================================================ */}
      {/*  Fallback sections when no rich raw data                      */}
      {/* ============================================================ */}
      {!hasRaw && (
        <>
          <SectionCard
            title="오늘의 인사이트"
            description="오늘은 성격을 설명하는 문장보다, 쓰는 방식이 중요합니다."
          >
      ...
      {/* Always show special tip if present from API */}
      {hasRaw && result.specialTip && (
        <SectionCard title="특별 메시지">
          <InsetQuote text={result.specialTip} />
        </SectionCard>
      )}
