...
const CLOUD_ONLY_FORTUNE_TYPES = new Set<FortuneTypeId>([
  // 전생 결과는 서버 Edge Function 이 Gemini/Nano Banana 계열 이미지 생성으로
...
export function isOnDeviceFortuneSupported(fortuneType: FortuneTypeId): boolean {
  if (CLOUD_ONLY_FORTUNE_TYPES.has(fortuneType)) return false;
