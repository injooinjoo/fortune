...
export const embeddedResultSeedByKind: Record<ResultKind, EmbeddedResultSeed> = {
  ...
  'face-reading': {
    ...
    metrics: [
      ...
      { label: '카리스마', value: '74', note: '존재감 점수' },
      { label: '친근감', value: '85', note: '다가가기 쉬운 인상' },
    ],
    highlights: ['전체적으로 균형 잡힌 이목구비가 안정적인 인상을 줍니다', '눈과 입의 조화가 신뢰감을 높이는 포인트입니다'],
    recommendations: ['밝은 색 계열의 옷이 인상을 부드럽게 만듭니다', '미소를 자주 지으면 친근감이 더 올라갑니다'],
    warnings: ['피로가 쌓이면 인상이 날카로워질 수 있으니 컨디션 관리가 중요합니다'],
  ...
  'beauty-simulation': {
    score: 80,
    summary: '스타일링 변화로 인상이 어떻게 달라지는지 한 컷 가이드로 보여줍니다.',
    metrics: [{ label: '추천 스타일', value: '3종', note: '베이스/포인트/시즌' }],
    highlights: ['지금 인상 강점을 살리는 메이크업 포인트를 정리합니다'],
    recommendations: ['추천 톤을 작은 부분에 먼저 적용해 보세요'],
    warnings: ['시뮬레이션은 참고 이미지이며 실제 결과는 다를 수 있어요'],
  },
  'hair-style-guide': {
    score: 81,
    summary: '얼굴형과 분위기에 어울리는 헤어스타일 가이드를 시각화합니다.',
    metrics: [{ label: '추천 컷', value: '3종', note: '톤·길이·결' }],
    highlights: ['얼굴 비례를 살리는 컷 라인이 함께 안내됩니다'],
    recommendations: ['추천 컷 사진을 미용실에서 함께 보여주세요'],
    warnings: ['실제 시술 결과는 모발 상태에 따라 차이가 있을 수 있어요'],
  },
  'face-reading-guide': {
    score: 82,
    summary: '관상 핵심 포인트를 카드 한 장으로 정리한 한국어 가이드입니다.',
    metrics: [{ label: '핵심 포인트', value: '5개', note: '오관·삼정·기색' }],
    highlights: ['전체 인상의 강점과 보완 포인트를 함께 짚어줍니다'],
    recommendations: ['좋은 인상을 만드는 표정 습관을 한 가지부터 시작해 보세요'],
    warnings: ['관상 가이드는 참고 자료입니다. 운명을 단정하지 않아요'],
  },
  'ootd-guide': {
    score: 80,
    summary: '오늘 분위기와 일정에 어울리는 코디 가이드를 한 장으로 보여줍니다.',
    metrics: [{ label: '룩 추천', value: '3종', note: '캐주얼/포멀/시즌' }],
    highlights: ['포인트 컬러와 소재 조합을 같이 안내합니다'],
    recommendations: ['추천 코디 중 하나만 골라 가볍게 시도해 보세요'],
    warnings: ['실내외 온도 차에 따라 레이어링을 조정하세요'],
  },
  'blind-date-guide': {
    score: 79,
    summary: '소개팅 흐름을 자연스럽게 만드는 톤·태도·복장을 카드로 정리합니다.',
    metrics: [{ label: '핵심 팁', value: '3개', note: '톤·복장·대화' }],
    highlights: ['긴장 풀고 호감을 높이는 첫인상 포인트를 안내합니다'],
    recommendations: ['먼저 가벼운 호의 표현 한 마디를 준비해 가세요'],
    warnings: ['지나친 준비는 오히려 부담이 될 수 있어요. 편한 모습이 가장 좋아요'],
  },
  'past-life-guide': {
    score: 81,
    summary: '전생에서 이어진 결을 한 장의 가이드 카드로 풀어 봅니다.',
    metrics: [{ label: '전생 키워드', value: '3개', note: '직업·성향·인연' }],
    highlights: ['지금 끌리는 분야와 전생의 흐름이 어떻게 이어지는지 안내합니다'],
    recommendations: ['끌리는 키워드 한 가지를 일상에 가볍게 들여보세요'],
    warnings: ['전생 가이드는 영감 자료로만 활용하세요'],
  },
  naming: {
    score: 90,
    summary: '사주 오행 분석을 바탕으로 아기에게 가장 어울리는 이름을 추천합니다.',
    metrics: [
      { label: '최고 점수', value: '95', note: '추천 이름 중 최고점' },
      { label: '용신', value: '火(화)', note: '부족한 오행 보완' },
    ],
    highlights: ['오행 균형을 고려한 이름 추천', '발음오행과 수리오행의 조화를 분석'],
    recommendations: ['출산 후 실제 사주로 최종 확정하세요', '출생신고 시 인명용 한자 범위를 확인하세요'],
    warnings: ['출산예정일 기준 분석이므로 실제 출생일과 다를 수 있습니다'],
    specialTip: '이름은 평생 부르는 소리입니다. 발음의 부드러움과 의미의 깊이가 조화를 이루는 이름이 좋습니다.',
  },
  birthstone: {
    score: 81,
    summary: '탄생석의 기운은 지금 타이밍과 잘 맞고, 보석이 가진 에너지를 활용하면 하루 흐름이 더 선명해집니다.',
    metrics: [
      { label: '탄생석 공명', value: '81%', note: '오늘의 보석 에너지' },
      { label: '행운 밀도', value: '높음', note: '탄생석 기운 상승기' },
    ],
  ...
  celebrity: {
    score: 85,
    summary: '사주 오행이 연결하는 두 사람의 기운 — 전생의 인연부터 케미 분석까지 풀어봅니다.',
    metrics: [
      { label: '궁합 점수', value: '85', note: '오행 기반 종합 궁합' },
      { label: '궁합 등급', value: '좋은 궁합', note: '사주 궁합 등급' },
    ],
    highlights: ['오행의 상호작용이 두 사람 사이에 특별한 에너지를 만들어냅니다'],
    recommendations: ['사주가 알려주는 최적의 타이밍에 마음을 열어보세요'],
    warnings: ['궁합은 참고 지표입니다 — 진짜 인연은 노력에서 완성됩니다'],
    luckyItems: ['행운의 시간대', '공통 관심사'],
    specialTip: '전생에서 이어진 인연의 실은 현생에서도 끊어지지 않습니다.',
  },
  'pet-compatibility': {
    score: 82,
    summary: '반려동물과의 유대감이 안정적으로 올라가는 흐름이고, 오늘은 함께 보내는 시간의 질이 관계 만족도를 크게 좌우합니다.',
    metrics: [
      { label: '유대감', value: '82', note: '교감 포인트 상승기' },
      { label: '컨디션', value: '79', note: '에너지 보통, 무리 금지' },
    ],
    highlights: ['오늘은 산책보다 집 안에서의 교감이 더 효과적인 흐름입니다'],
    recommendations: ['짧은 눈 맞춤과 부드러운 터치를 자주 해보세요', '간식보다 놀이 시간을 늘리는 편이 좋습니다'],
    warnings: ['과한 자극이나 큰 소리는 피해주세요'],
    luckyItems: ['따뜻한 색 담요', '부드러운 간식', '오후 산책'],
    specialTip: '반려동물은 주인의 감정을 가장 먼저 읽습니다. 오늘은 편안한 마음으로 함께 있는 것만으로도 최고의 교감이 됩니다.',
  },
  'lucky-items': {
    score: 80,
    summary: '오행 분석 결과 오늘은 부족한 기운을 보완하면 행운이 따르는 흐름입니다. 색상과 방향 선택이 하루 흐름을 크게 바꿉니다.',
    metrics: [
      { label: '행운 지수', value: '80', note: '오행 보완 시 상승' },
      { label: '핵심 오행', value: '금(金)', note: '보완 필요 오행' },
    ],
    highlights: ['오늘은 금 기운이 부족하므로 흰색 계열과 금속 소재가 도움됩니다'],
    recommendations: ['행운 색상을 옷이나 소품에 활용해보세요', '추천 방향으로 이동하면 기운이 강해집니다'],
    warnings: ['피해야 할 숫자나 방향은 에너지를 분산시킬 수 있어요'],
    luckyItems: ['실버 액세서리', '흰색 셔츠', '동남쪽 산책'],
    specialTip: '행운은 믿는 만큼 가까워집니다. 오늘 추천 아이템 하나만 실천해도 하루 흐름이 달라질 수 있어요.',
  },
  moving: {
    score: 81,
    summary: '이사 방위와 시기를 종합하면 안정적인 흐름이며, 풍수 배치와 손없는 날을 맞추면 정착 운이 크게 올라갑니다.',
    metrics: [
      { label: '방위 궁합', value: '81', note: '동남쪽 기운 양호' },
      { label: '정착 지수', value: '78', note: '첫 주 적응이 핵심' },
    ],
    highlights: ['이사 방향의 오행 기운이 안정적으로 흐르는 구간입니다', '손없는 날을 맞추면 정착 속도가 빨라집니다'],
    recommendations: ['현관에 소금과 쌀을 먼저 들여놓으세요', '이사 당일 오전 시간대가 가장 좋습니다'],
