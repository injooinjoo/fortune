...
### 5. No Defer — "다음 단계", "추후" 같은 미루기 금지
사용자에게 "다음에 할 것", "별도 작업으로", "추후 모니터링", "다음 라운드" 같은 미루기 표현 금지. 보고에 들어갈 모든 항목은 **응답 전에 끝낸 상태**여야 함.

미룬다는 건 = 끝까지 안 했다는 것. 작업이 너무 크면 명시적으로 "scope 확장 필요, 먼저 X 부터 해도 되냐" 라고 push back. **"이건 다음에" 라고 listing 만 하지 말 것**. 만약 진짜 외부 의존 (사용자만 할 수 있는 API key 설정, 사용자 결정 필요 등) 으로 못 끝내면, 그 의존을 정확히 명시 ("X 환경변수 필요" / "Y 결정 후 진행").

작동 신호: 응답에 "다음 단계", "추후", "별도 작업", "monitoring", "TBD" 같은 말이 들어가면 자가 점검 — 정말 못 하는 건지, 아니면 게으름 / 두려움 / 시간 절약 시도인지. 두 번째 케이스면 그냥 함.
...
로컬 native 테스트 명령(Expo Go/터널/EAS 없이, 사용자 확인용):
- 준비/Pods: `pnpm rn:native:prepare`
- 로컬 빌드 검증: `pnpm rn:native:build`
- Xcode 열기: `pnpm rn:ios:xcode`
- Metro: `pnpm rn:start:native`
- CLI 실행: `pnpm rn:ios:local`
- 상세 문서: `docs/development/local-native-ios-testing.md`
