# [3.6] 관상 닮은꼴 분석 기능 기획서

## 1. 요구사항

| 항목 | 설명 | 복잡도 |
|------|------|--------|
| 3.6.1 | 관상 분석 결과에 닮은꼴 유명인 추가 | 높음 |
| 3.6.2 | 유사도 점수 및 닮은 부위 표시 | 중간 |

---

## 2. 현황 분석

### 2.1 현재 구현

**관상 페이지 (face_reading_page.dart)**:
- 사진 업로드 → 관상 분석 → 텍스트 결과
- 닮은꼴 분석 기능 없음
- 결과: 이마, 눈썹, 눈, 코, 입, 전체 인상

**관상운세 페이지 (face_reading_fortune)**:
- 사진 업로드 → 상세 관상 분석 → 닮은꼴 유명인 포함
- `CelebrityMatchCarousel` 위젯 사용
- Edge Function: `similar_celebrities` 반환

### 2.2 관련 파일
```
lib/features/interactive/presentation/pages/
└── face_reading_page.dart (대상 페이지)

lib/features/fortune/presentation/pages/face_reading_fortune/
├── face_reading_result_page.dart (참조 - 닮은꼴 포함)
└── widgets/
    └── result_widget.dart

lib/features/fortune/presentation/widgets/face_reading/
└── celebrity_match_carousel.dart (재사용 가능)
```

### 2.3 CelebrityMatchCarousel 데이터 구조

```dart
// API 응답 예시
{
  "similar_celebrities": [
    {
      "celebrity_name": "아이유",
      "celebrity_type": "solo_singer",
      "similarity_score": 87,
      "matched_features": ["눈매", "웃는 모습", "이목구비 비율"],
      "character_image_url": "https://...",
      "reason": "맑고 또렷한 눈매가 매우 유사합니다"
    },
    // ...
  ]
}
```

### 2.4 현재 face_reading_page 구조

```
┌─────────────────────────────────────┐
│  관상                         [←]   │
├─────────────────────────────────────┤
│  ┌─────────────────────────────┐    │
│  │    당신의 얼굴을 분석합니다   │    │
│  │    5 토큰 필요              │    │
│  └─────────────────────────────┘    │
│                                     │
│  ┌─────────────────────────────┐    │
│  │      [사진 영역]            │    │
│  │                             │    │
│  └─────────────────────────────┘    │
│                                     │
│  [📷 카메라] [🖼️ 갤러리]           │
│                                     │
│  ─────── 결과 (분석 후) ──────      │
│  【이마】 넓고 시원한 이마는...      │
│  【눈썹】 짙고 선명한 눈썹은...      │
│  【눈】 맑고 깊은 눈은...           │
│  【코】 균형 잡힌 코는...           │
│  【입】 따뜻한 미소가...            │
│  【전체】 전반적으로...             │
│                                     │
│  [다시 분석] [결과 공유]            │
└─────────────────────────────────────┘
```

---

## 3. 개선 방향

### 3.1 목표

기존 관상 결과에 **닮은꼴 유명인 섹션** 추가:
- CelebrityMatchCarousel 위젯 재사용
- 스와이프 가능한 캐러셀 형태
- 유사도 점수, 닮은 부위, 이유 표시

### 3.2 개선 UI 레이아웃

```
┌─────────────────────────────────────┐
│  관상                         [←]   │
├─────────────────────────────────────┤
│  ... (기존 입력/사진 영역)           │
│                                     │
│  ─────── 결과 (분석 후) ──────      │
│                                     │
│  ┌── 닮은꼴 유명인 (NEW) ──┐        │
│  │ ⭐ 닮은꼴 유명인   1/3  │        │ ← 캐러셀 헤더
│  │                         │        │
│  │ ┌─────────────────────┐ │        │
│  │ │ [🏆1] 아이유        │ │        │ ← 캐러셀 카드
│  │ │ 솔로 가수           │ │        │
│  │ │ ⭐ 유사도 87%       │ │        │
│  │ │ 눈매 | 웃는 모습    │ │        │
│  │ │ "맑고 또렷한..."    │ │        │
│  │ └─────────────────────┘ │        │
│  │     ○ ● ○              │        │ ← 도트 인디케이터
│  └─────────────────────────┘        │
│                                     │
│  【이마】 넓고 시원한 이마는...      │
│  【눈썹】 짙고 선명한 눈썹은...      │
│  ... (기존 분석 결과)               │
│                                     │
└─────────────────────────────────────┘
```

---

## 4. 상세 구현

### 4.1 Import 추가

```dart
// face_reading_page.dart
import '../../../fortune/presentation/widgets/face_reading/celebrity_match_carousel.dart';
```

### 4.2 상태 변수 추가

```dart
class _FaceReadingPageState extends ConsumerState<FaceReadingPage> {
  // 기존 변수들...
  String? _analysisResult;

  // ✅ 닮은꼴 유명인 데이터 추가
  List<Map<String, dynamic>>? _similarCelebrities;
}
```

### 4.3 분석 API 수정

현재 `_analyzeImage()`는 하드코딩된 결과를 반환합니다.
실제 API 호출로 변경하거나, 닮은꼴 데이터도 포함하도록 수정:

```dart
Future<void> _analyzeImage() async {
  // 토큰 확인...
  setState(() => _isAnalyzing = true);

  try {
    // TODO: 실제 API 호출
    // 옵션 1: 기존 face-reading Edge Function 확장
    // 옵션 2: face-reading-fortune Edge Function 사용

    final response = await UnifiedFortuneService(Supabase.instance.client)
        .getFortune(
          fortuneType: 'face-reading',
          inputConditions: {
            'imageBase64': base64Encode(await _selectedImage!.readAsBytes()),
            'includeCelebrityMatch': true, // 닮은꼴 분석 포함
          },
        );

    // 토큰 차감...

    setState(() {
      _analysisResult = _formatAnalysisResult(response);
      _similarCelebrities = (response.data['similar_celebrities'] as List?)
          ?.map((e) => e as Map<String, dynamic>)
          .toList();
      _isAnalyzing = false;
    });
  } catch (e) {
    // 에러 처리...
  }
}
```

### 4.4 결과 섹션에 캐러셀 추가

```dart
Widget _buildResultSection() {
  return GlassContainer(
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // 헤더
        Row(
          children: [
            Icon(Icons.psychology, color: Theme.of(context).colorScheme.primary),
            const SizedBox(width: 8),
            Text('관상 분석 결과', style: headlineStyle),
          ],
        ),
        const SizedBox(height: 16),

        // ✅ 닮은꼴 유명인 캐러셀 추가
        if (_similarCelebrities != null && _similarCelebrities!.isNotEmpty) ...[
          CelebrityMatchCarousel(
            celebrities: _similarCelebrities!,
          ),
          const SizedBox(height: 24),
        ],

        // 기존 분석 결과
        Text(
          _analysisResult!,
          style: bodyStyle,
        ),

        // 액션 버튼들...
      ],
    ),
  );
}
```

---

## 5. Edge Function 수정 (필요 시)

### 5.1 face-reading Edge Function 확장

현재 `face-reading` Edge Function이 닮은꼴 분석을 지원하지 않는다면:

**옵션 A**: 기존 함수 확장
```typescript
// supabase/functions/face-reading/index.ts

interface FaceReadingResponse {
  // 기존 필드들...
  forehead: string;
  eyebrows: string;
  eyes: string;
  nose: string;
  mouth: string;
  overall: string;

  // ✅ 닮은꼴 추가
  similar_celebrities?: {
    celebrity_name: string;
    celebrity_type: string;
    similarity_score: number;
    matched_features: string[];
    reason: string;
  }[];
}
```

**옵션 B**: face-reading-fortune 함수 재사용
- 이미 닮은꼴 분석이 구현된 함수 사용
- 불필요한 필드 필터링

---

## 6. 체크리스트

### 6.1 프론트엔드
- [ ] CelebrityMatchCarousel import 추가
- [ ] _similarCelebrities 상태 변수 추가
- [ ] _analyzeImage에서 닮은꼴 데이터 파싱
- [ ] _buildResultSection에 캐러셀 추가
- [ ] flutter analyze 검증

### 6.2 백엔드 (필요 시)
- [ ] face-reading Edge Function 확장
- [ ] 닮은꼴 분석 로직 추가
- [ ] 유명인 DB 연동

---

## 7. 예상 소요 시간

| 항목 | 예상 시간 |
|------|----------|
| Import 및 상태 변수 추가 | 10분 |
| _analyzeImage 수정 | 30분 |
| 결과 UI에 캐러셀 추가 | 20분 |
| Edge Function 수정 (필요 시) | 60분 |
| 테스트 | 20분 |
| **합계 (프론트만)** | **60분** |
| **합계 (백엔드 포함)** | **140분** |

---

## 8. 앱 내 유사 구현 참조

### CelebrityMatchCarousel
```
lib/features/fortune/presentation/widgets/face_reading/celebrity_match_carousel.dart
```
- 스와이프 가능한 캐러셀
- 유명인 카드 UI
- 타입별 색상 처리
- 순위 배지

### FaceReadingResultPage
```
lib/features/fortune/presentation/pages/face_reading_fortune/face_reading_result_page.dart
```
- 닮은꼴 캐러셀 사용 예시
- API 응답 파싱 패턴

### CelebrityService
```
lib/services/celebrity_service.dart
```
- 유명인 검색/조회 서비스
- 이미지 URL 관리

---

## 9. 추가 고려사항

### 9.1 프리미엄 기능 여부
- 닮은꼴 분석을 프리미엄 전용으로 할 것인지?
- 토큰 소비로 처리?

### 9.2 유명인 DB
- 현재 유명인 DB가 충분한지 확인
- 부족하면 celebrity_crawling_service 활용

### 9.3 이미지 처리
- 얼굴 인식 정확도
- 비교 알고리즘 (AI 기반)

---

**작성일**: 2025-12-17
