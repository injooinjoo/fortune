# [3.8] 부적 유료화 기획서

## 1. 요구사항

| 항목 | 설명 | 복잡도 |
|------|------|--------|
| 3.8.1 | 부적 생성 전 광고 시청 또는 복채(토큰) 차감 | 중간 |
| 3.8.2 | 프리미엄 구독자는 무제한 이용 | 낮음 |

---

## 2. 현황 분석

### 2.1 현재 구현
- 부적 생성: 무료 (제한 없음)
- AI 이미지 생성: 선택 사항
- 토큰 시스템: 다른 운세에서 사용 중

### 2.2 관련 파일
```
lib/features/fortune/presentation/pages/
└── talisman_fortune_page.dart

lib/presentation/providers/
├── token_provider.dart (토큰 관리)
└── subscription_provider.dart (구독 상태)

lib/services/
└── ad_service.dart (광고 서비스)
```

### 2.3 기존 유료화 패턴 (다른 운세 참조)

**Face Reading (관상)**:
```dart
static const int _requiredTokens = 5;

// 토큰 확인
final tokenBalance = ref.read(tokenBalanceProvider);
if (tokenBalance.remainingTokens < _requiredTokens &&
    !tokenBalance.hasUnlimitedAccess) {
  _showInsufficientTokensModal();
  return;
}

// 토큰 차감
await ref.read(tokenProvider.notifier).consumeTokens(
  fortuneType: 'face_reading',
  amount: _requiredTokens,
);
```

**MBTI (토큰 기반)**:
```dart
// 토큰 차감
await ref.read(tokenProvider.notifier).consumeTokens(
  fortuneType: 'mbti',
  amount: 3,
);
```

---

## 3. 유료화 모델 설계

### 3.1 가격 정책

| 방식 | 비용 | 설명 |
|------|------|------|
| 광고 시청 | 무료 | 리워드 광고 시청 후 1회 생성 |
| 복채 (토큰) | 3개 | 토큰으로 즉시 생성 |
| 구독자 | 무료 | 무제한 생성 |

### 3.2 UI 흐름 (선결제 후 입력)

```
부적 진입 → 유료화 게이트 → 카테고리 선택 → 소원 입력 → 생성
              ↓
    ┌─────────────────────┐
    │   부적 만들기        │
    ├─────────────────────┤
    │                     │
    │ 소원을 담은 부적을   │
    │ 만들어 드릴게요     │
    │                     │
    │ [📺 광고 보고       │
    │   무료로 시작하기]  │
    │                     │
    │ [🪙 복채 3개로      │
    │   바로 시작하기]    │
    │                     │
    │ [⭐ 프리미엄        │
    │   무제한 이용하기]  │
    │                     │
    └─────────────────────┘

※ 구독자는 게이트 없이 바로 진입
※ 결제 완료 후 자유롭게 입력/생성
```

### 3.3 구독자 처리

```dart
// 구독자는 바로 생성
if (ref.read(isPremiumProvider)) {
  _generateTalisman();
  return;
}

// 비구독자는 게이트 표시
_showPaymentGate();
```

---

## 4. 상세 구현

### 4.1 결제 게이트 위젯

```dart
class TalismanPaymentGate extends ConsumerWidget {
  final VoidCallback onAdWatched;
  final VoidCallback onTokenPaid;
  final VoidCallback onPremiumSelected;

  static const int requiredTokens = 3;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final tokenBalance = ref.watch(tokenBalanceProvider);
    final hasEnoughTokens = (tokenBalance?.remainingTokens ?? 0) >= requiredTokens;

    return Container(
      padding: EdgeInsets.all(24),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text('부적 생성하기', style: headlineStyle),
          SizedBox(height: 8),
          Text('소원을 담은 부적을 만들어 드릴게요'),
          SizedBox(height: 24),

          // 광고 옵션
          _buildOptionCard(
            icon: Icons.play_circle_fill,
            title: '광고 보고 무료로 만들기',
            subtitle: '30초 광고 시청',
            onTap: onAdWatched,
          ),

          SizedBox(height: 12),

          // 토큰 옵션
          _buildOptionCard(
            icon: Icons.toll,
            title: '복채 ${requiredTokens}개로 바로 만들기',
            subtitle: hasEnoughTokens
              ? '보유: ${tokenBalance?.remainingTokens ?? 0}개'
              : '토큰 부족',
            onTap: hasEnoughTokens ? onTokenPaid : null,
            enabled: hasEnoughTokens,
          ),

          SizedBox(height: 12),

          // 구독 옵션
          _buildOptionCard(
            icon: Icons.workspace_premium,
            title: '프리미엄 구독하기',
            subtitle: '무제한 부적 생성',
            onTap: onPremiumSelected,
            isPremium: true,
          ),
        ],
      ),
    );
  }
}
```

### 4.2 광고 처리

```dart
Future<void> _handleAdOption() async {
  final adService = AdService();

  if (!adService.isRewardedAdReady) {
    // 로딩 표시
    _showLoadingDialog('광고를 준비하고 있어요...');
    await adService.loadRewardedAd();
    Navigator.pop(context);
  }

  await adService.showRewardedAd(
    onUserEarnedReward: (ad, reward) {
      _generateTalisman();
    },
  );
}
```

### 4.3 토큰 처리

```dart
Future<void> _handleTokenOption() async {
  try {
    await ref.read(tokenProvider.notifier).consumeTokens(
      fortuneType: 'talisman',
      amount: TalismanPaymentGate.requiredTokens,
    );
    _generateTalisman();
  } catch (e) {
    Toast.show(context, message: '토큰 차감에 실패했습니다', type: ToastType.error);
  }
}
```

### 4.4 talisman_fortune_page.dart 수정

```dart
// 기존: 바로 생성
void _onGeneratePressed() {
  _generateTalisman();
}

// 변경: 유료화 게이트 추가
void _onGeneratePressed() {
  // 구독자는 바로 생성
  if (ref.read(isPremiumProvider)) {
    _generateTalisman();
    return;
  }

  // 비구독자는 게이트 표시
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    builder: (context) => TalismanPaymentGate(
      onAdWatched: () {
        Navigator.pop(context);
        _handleAdOption();
      },
      onTokenPaid: () {
        Navigator.pop(context);
        _handleTokenOption();
      },
      onPremiumSelected: () {
        Navigator.pop(context);
        _navigateToSubscription();
      },
    ),
  );
}
```

---

## 5. 체크리스트

- [ ] TalismanPaymentGate 위젯 생성
- [ ] 광고 처리 로직 (_handleAdOption)
- [ ] 토큰 처리 로직 (_handleTokenOption)
- [ ] talisman_fortune_page.dart에 게이트 연동
- [ ] isPremiumProvider 체크 추가
- [ ] 토큰 부족 시 UI 처리
- [ ] 구독 페이지 연결
- [ ] flutter analyze 검증

---

## 6. 예상 소요 시간

| 항목 | 예상 시간 |
|------|----------|
| TalismanPaymentGate 위젯 | 30분 |
| 광고 처리 로직 | 15분 |
| 토큰 처리 로직 | 15분 |
| 페이지 연동 | 20분 |
| 테스트 | 15분 |
| **합계** | **95분** |

---

## 7. 앱 내 유사 구현 참조

### TokenInsufficientModal
```
lib/shared/components/token_insufficient_modal.dart
```
- 토큰 부족 시 표시되는 모달
- 광고/구독 옵션 제공

### Face Reading 유료화
```
lib/features/interactive/presentation/pages/face_reading_page.dart
```
- _requiredTokens, _analyzeImage()
- 토큰 차감 패턴

### MBTI 토큰 소비
```
lib/features/fortune/presentation/pages/mbti_fortune/mbti_fortune_page.dart
```
- 토큰 차감 패턴

---

**작성일**: 2025-12-17