...
> 최종 업데이트: 2025.01.03
>
> ⚠️ **RN 마이그레이션 후 본문 갱신 대기**: 본 문서의 코드 예제는 Flutter `context.heading*` API 기준입니다. 현재 RN 구현은 `<AppText variant="...">` 패턴 (CLAUDE.md "핵심 패턴 — Typography" 참고)을 씁니다. **정책/원칙만 신뢰**하고 코드 예제는 RN 매핑이 필요합니다.
