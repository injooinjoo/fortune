...
export class ImprovedOpenAIClient {
  ...
  async generateFortune({
    ...
    // All retries failed, report to Sentry and throw
    if (lastError) {
      Sentry.withScope((scope) => {
        scope.setContext('openai_failure', {
...
// Singleton instance
let openaiClient: ImprovedOpenAIClient | null = null;

export function getOpenAIClient(): ImprovedOpenAIClient {
  if (!openaiClient) {
    openaiClient = new ImprovedOpenAIClient();
  }
  return openaiClient;
}

// Export for testing
export { ImprovedOpenAIClient };