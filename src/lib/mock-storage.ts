...
export function saveUserProfile(userId: string, profile: any) {
  ...
  logger.debug('🔵 프로필 저장됨:', { userId, profile });
  logger.debug('🔵 저장 후 전체 프로필들:', userProfiles);
  logger.debug('🔵 글로벌 저장소 상태:', global.__MOCK_STORAGE__);
}

// 프로필 조회
export function getUserProfile(userId: string) {
  logger.debug('🔍 프로필 조회 시작:', { userId });
  logger.debug('🔍 현재 전체 프로필들:', userProfiles);
  logger.debug('🔍 글로벌 저장소 상태:', global.__MOCK_STORAGE__);
  
  const profile = userProfiles[userId];
  logger.debug('🔍 프로필 조회 결과:', { userId, found: !!profile, profile });
  return profile;
}

// 모든 프로필 조회 (디버깅용)
export function getAllProfiles() {
  logger.debug('📋 전체 프로필 조회:', userProfiles);
  logger.debug('📋 글로벌 저장소:', global.__MOCK_STORAGE__);
  return userProfiles;
} 