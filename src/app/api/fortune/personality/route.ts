...
export const GET = withFortuneAuth(async (request: AuthenticatedRequest, fortuneService: FortuneService) => {
  try {
    ...
    logger.debug('✅ 성격 분석 API 응답 완료:', userId);
    ...
    logger.error('❌ 성격 분석 API 오류:', error);
    
    return createSafeErrorResponse(error, '성격 분석을 가져오는 중 오류가 발생했습니다.');
  }
});
