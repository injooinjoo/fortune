...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('✅ 주간 운세 API 응답 완료:', userId);
    ...
    console.error('❌ 주간 운세 API 오류:', error);
    
    return NextResponse.json(
      { 
        success: false, 
        error: '주간 운세를 가져오는 중 오류가 발생했습니다.' 
      },
      { status: 500 }
    );
  }
