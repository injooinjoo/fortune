...
export const GET = withFortuneAuth(async (request: AuthenticatedRequest, fortuneService: FortuneService) => {
  try {
    ...
    logger.debug('✅ 전통 사주 API 응답 완료:', userId);
    ...
    logger.error('❌ 전통 사주 API 오류:', error);
    
    return createSafeErrorResponse(error, '전통 사주를 가져오는 중 오류가 발생했습니다.');
  }
});
