...
export const GET = withFortuneAuth(async (request: AuthenticatedRequest, fortuneService: FortuneService) => {
  try {
    logger.debug('📅 내일의 운세 API 요청');
    
    // URL에서 사용자 ID 추출 (테스트용)
    const { searchParams } = new URL(request.url);
    const userId = request.userId!; // 동적 기본값
    
    logger.debug(`🔍 내일의 운세 요청: 사용자 ID = ${userId}`);
    ...
    logger.debug('✅ 내일의 운세 API 응답 완료:', userId);
    ...
    logger.error('❌ 내일의 운세 API 오류:', error);
    
    return createSafeErrorResponse(error, '내일의 운세를 가져오는 중 오류가 발생했습니다.');
  }
});
