...
class FortuneTextCleaner {
  ...
  static String clean(String content) {
    ...
    cleaned = cleaned.replaceAll(RegExp(r'안녕하세요[,.]?\s*당신의\s*앞날을\s*밝혀주는[^\n]*상담가입니다[.。]?\s*'), '');
    cleaned = cleaned.replaceAll(RegExp(r'오늘은\s*\d+점의[^\n]*기다리고\s*있네요[!！]?\s*'), '');
    cleaned = cleaned.replaceAll(RegExp(r'오늘\s*하루[,.]?\s*긍정적인\s*에너지를\s*듬뿍\s*담아[^\n]*만들어\s*보세요[.。]?\s*'), '');
    cleaned = cleaned.replaceAll(RegExp(r'오늘의\s*키워드는\s*바로[^\n]*덕목입니다[.。]?\s*'), '');
