...
class CacheService {
  ...
  Future<void> cacheFortune(Fortune fortune) async {
    try {
      ...
      Logger.debug('Cached fortune: $cacheKey');
    } catch (e) {
  ...
  Future<Fortune?> getCachedFortune(String fortuneType, String userId) async {
    try {
      ...
      if (cachedFortune.isExpired) {
        ...
        Logger.debug('Removed expired fortune: $cacheKey');
        return null;
      }
      
      Logger.debug('Retrieved cached fortune: $cacheKey');
  ...
  Future<void> clearFortuneCache({String? userId}) async {
    try {
      if (userId != null) {
        ...
        Logger.debug('Cleared fortune cache for user: $userId');
      } else {
