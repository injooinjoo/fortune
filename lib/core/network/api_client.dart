...
class ApiClient {
  ...
  ApiClient() {
    ...
    _dio.interceptors.addAll([
      ...
      InterceptorsWrapper(onRequest: (options, handler) {
        ...
        final requestTime =
            error.requestOptions.extra['requestTime'] as DateTime?;
        final responseTime = requestTime != null
            ? DateTime.now().difference(requestTime).inMilliseconds
            : null;
        ...
        if (statusCode == 404 && isOptionalEndpoint) {
          Logger.debug(
              '🔍 [ApiClient] Optional endpoint not found (gracefully handled)',
              {
                'endpoint': uri,
          ...
          if (error.requestOptions.uri.toString().contains('/fortune/')) {
            final fortuneType =
                error.requestOptions.uri.pathSegments.lastOrNull ?? 'unknown';
  ...
  // 스트리밍 응답 처리 (운세 생성용,
  Stream<String> getStream(String path,
      {Map<String, dynamic>? queryParameters,
  ...
  Exception _handleError(dynamic error) {
          ...
          if (data is Map<String, dynamic>) {
            ...
            if (statusCode == 402 || data['code'] == 'insufficient_tokens') {
              return app_exceptions.TokenException(
                  message: message, remainingTokens: data['remainingTokens']);
            }

            // 유효성 검사 에러
            if (statusCode == 400) {
              return app_exceptions.ValidationException(
                  message: message, errors: data['errors']);
            }
    ...
    return app_exceptions.ServerException(message: error.toString());
  }
}
