...
class NativeFeaturesInitializer extends ResilientService {
  ...
  static final NativeFeaturesInitializer _instance =
  ...
  Future<void> _initializeInternal() async {
    ...
    ], false, '네이티브 기능 종합 초기화', '네이티브 기능 비활성화');
  ...
  Future<bool> _initializeWidgetServices() async {
    return await safeExecuteWithBool(() async {
      ...
      }, '초기 위젯 데이터 처리', '초기 위젯 데이터 무시');
    }, '위젯 서비스 초기화', '위젯 기능 비활성화');
  }

  static Future<bool> requestPermissions() async =>
      _instance._requestPermissionsInternal();
  ...
  Future<bool> _requestPermissionsInternal() async {
    ...
    }, '알림 권한 요청', '권한 없이 계속 진행');
  }

  Future<void> _registerBackgroundTasks() async {
    await safeExecute(() async {
      await Workmanager().registerPeriodicTask(
          'widget_update_task', 'widget_update',
          frequency: const Duration(hours: 1),
          constraints: Constraints(networkType: NetworkType.connected));
    }, '위젯 업데이트 백그라운드 작업 등록', '위젯 자동 업데이트 비활성화');

    await safeExecute(() async {
      await Workmanager().registerPeriodicTask(
          'daily_fortune_task', 'daily_fortune_fetch',
          frequency: const Duration(days: 1),
          initialDelay: _calculateInitialDelay(6, 0),
          constraints: Constraints(networkType: NetworkType.connected));
    }, '일일 운세 백그라운드 작업 등록', '일일 운세 자동 갱신 비활성화');

    // 아침 배지 표시 태스크 (06:00, 푸시 없이 조용히)
    await safeExecute(() async {
      await Workmanager().registerPeriodicTask(
          'morning_badge_task', 'morning_badge_check',
          frequency: const Duration(days: 1),
          initialDelay: _calculateInitialDelay(6, 0),
          constraints: Constraints(networkType: NetworkType.notRequired));
    }, '아침 배지 백그라운드 작업 등록', '아침 배지 자동 표시 비활성화');
  ...
  static Future<void> scheduleDailyNotification({
    ...
    bool enabled = true,
      ...
      _instance._scheduleDailyNotificationInternal(hour, minute, enabled);

  Future<void> _scheduleDailyNotificationInternal(
    ...
    }, '일일 운세 알림 예약', '알림 예약 실패, 수동 확인 필요');
  }

  static Future<void> showTestNotification() async =>
      _instance._showTestNotificationInternal();
  ...
  static Future<void> updateLoveFortuneWidget({
    ...
    required String message,
      ...
      _instance._updateLoveFortuneWidgetInternal(
          partnerName, compatibilityScore, message);

  Future<void> _updateLoveFortuneWidgetInternal(
      String partnerName, int compatibilityScore, String message) async {
    await safeExecute(() async {
