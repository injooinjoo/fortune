...
class RegionService extends ResilientService {
      ...
      [
        ...
        () async {
          if (_isCacheValid(cacheKey, _popularRegionsCacheDuration)) {
            Logger.info(
                '인기 지역 캐시 사용 (${(_cache[cacheKey] as List<Region>).length}개)');
  ...
  Future<List<Region>> searchRegions(String query) async {
    ...
    ], _searchLocalFallback(query), '지역 검색: $query', '모든 검색 실패, 로컬 데이터 검색 수행');
  }
  ...
  Future<List<Region>> _searchPopularRegions(String query) async {
    ...
    }, () async => <Region>[], '인기 지역 검색: $query', 'Supabase 검색 실패, 빈 결과 반환');
  }
  ...
  Future<List<Region>> _searchPublicApi(String query) async {
    try {
      ...
      if (apiKey.isEmpty || apiKey == 'YOUR_KAKAO_REST_API_KEY_HERE') {
        ...
        return [
          Region(
              displayName: '$query (API 키 없음)', sido: '설정필요', sigungu: query),
        ];
      }
  ...
  Future<void> incrementUsageCount(String displayName) async {
    await safeExecute(() async {
      await _supabase.from('popular_regions').update(
          {'usage_count': 'usage_count + 1'}).eq('display_name', displayName);

      Logger.info('지역 사용 횟수 증가: $displayName');
    }, '지역 사용 횟수 증가: $displayName', '분석 데이터 수집 실패 (무시됨, 기능은 정상 작동)');
  }
  ...
  Future<List<Region>> _getPopularRegionsFromKakao() async {
    ...
    Logger.info(
        '카카오 API 병렬 조회 완료 (${regions.length}개 성공, ${popularDistricts.length - regions.length}개 실패)');
    return regions;
  }
  ...
  List<Region> _searchLocalFallback(String query) {
    ...
    for (final district in seoulDistricts) {
      if (district.contains(queryLower) ||
          queryLower.contains('서울') ||
    ...
    if (queryLower.contains('대구')) {
      regions
          .add(Region(displayName: '대구시 수성구', sido: '대구광역시', sigungu: '수성구'));
      regions
          .add(Region(displayName: '대구시 달서구', sido: '대구광역시', sigungu: '달서구'));
    }

    // 인천 검색
    if (queryLower.contains('인천')) {
      regions
          .add(Region(displayName: '인천시 연수구', sido: '인천광역시', sigungu: '연수구'));
      regions
          .add(Region(displayName: '인천시 남동구', sido: '인천광역시', sigungu: '남동구'));
    }
