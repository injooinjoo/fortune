...
class GoogleAuthProvider extends BaseSocialAuthProvider {
  ...
  Future<AuthResponse?> signIn() async {
    try {
      debugPrint('🟡 [GoogleAuthProvider] signIn() started with Supabase OAuth');
      ...
      if (supabaseUrl.contains('your-project')) {
        Logger.warning('[GoogleAuthProvider] Supabase URL 설정 오류 (선택적 기능, 설정 확인 필요): Supabase URL이 제대로 설정되지 않음');
        throw Exception('Supabase URL is not properly configured');
      }

      debugPrint('🟡 [GoogleAuthProvider] Starting Supabase OAuth for Google...');
      ...
      Logger.warning('[GoogleAuthProvider] Google OAuth 실패 (선택적 기능, 다른 로그인 방법 사용 권장): $error');
      Logger.warning('[GoogleAuthProvider] Google OAuth 에러 타입 (선택적 기능, 다른 로그인 방법 사용 권장): ${error.runtimeType}');
      rethrow;
    }
  }
}
