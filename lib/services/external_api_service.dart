...
class ExternalApiService extends ResilientService {
  ...
  Future<WeatherInfo> getWeatherData(String location) async {
    ...
    }, '날씨 정보 조회: $location', '날씨 서비스 연결 실패, 기본 날씨 정보 제공');
  }
  ...
  Future<List<GameSchedule>> getBaseballSchedule(String team) async {
    ...
    }, '야구 일정 조회: $team', '야구 API 연결 실패, 기본 일정 데이터 제공');
  }

  /// Static 호환성을 위한 래퍼 메서드
  static Future<List<GameSchedule>> getBaseballScheduleStatic(
      String team) async {
  ...
  Future<List<EsportsMatch>> getLCKSchedule({String? team}) async {
    ...
    }, 'LCK 일정 조회: ${team ?? 'all'}', 'LCK API 연결 실패, 기본 일정 데이터 제공');
  }
  ...
  Future<List<GolfCourse>> getGolfCourseInfo(String region) async {
    ...
    }, '골프장 정보 조회: $region', '골프장 API 연결 실패, 기본 골프장 정보 제공');
  }
  ...
  Future<Map<String, dynamic>> getCryptoMarketData() async {
    ...
    }, '암호화폐 시세 조회', '암호화폐 API 연결 실패, 기본 시세 정보 제공');
  }
  ...
  Future<Map<String, dynamic>> getLottoStatistics() async {
    ...
    }, '로또 통계 조회', '로또 API 연결 실패, 기본 통계 정보 제공');
  }
  ...
  Future<void> clearCache() async {
    ...
    }, '외부 API 캐시 초기화', '캐시 초기화 실패');
  }

  /// Static 호환성을 위한 기존 메서드들 (Deprecated)
  @Deprecated('Use instance methods instead for better error handling')
  static Future<WeatherInfo> getWeatherDataOld(String location) =>
      _instance.getWeatherData(location);

  @Deprecated('Use instance methods instead for better error handling')
  static Future<List<GameSchedule>> getBaseballScheduleOld(String team) =>
      _instance.getBaseballSchedule(team);

  @Deprecated('Use instance methods instead for better error handling')
  static Future<List<EsportsMatch>> getLCKScheduleOld({String? team}) =>
      _instance.getLCKSchedule(team: team);

  @Deprecated('Use instance methods instead for better error handling')
  static Future<List<GolfCourse>> getGolfCourseInfoOld(String region) =>
      _instance.getGolfCourseInfo(region);

  @Deprecated('Use instance methods instead for better error handling')
  static Future<Map<String, dynamic>> getCryptoMarketDataOld() =>
      _instance.getCryptoMarketData();

  @Deprecated('Use instance methods instead for better error handling')
  static Future<Map<String, dynamic>> getLottoStatisticsOld() =>
      _instance.getLottoStatistics();
