...
class NativePlatformService extends ResilientService {
  static final NativePlatformService _instance =
  ...
  static const MethodChannel iosChannel =
      MethodChannel('com.fortune.fortune/ios');
  static const MethodChannel androidChannel =
      MethodChannel('com.fortune.fortune/android');

  // Event channels for receiving native updates
  static const EventChannel _iosEventChannel =
      EventChannel('com.fortune.fortune/ios/events');
  static const EventChannel _androidEventChannel =
      EventChannel('com.fortune.fortune/android/events');
  ...
  Future<void> _initializeInternal() async {
    ...
    }, '네이티브 플랫폼 초기화', '네이티브 기능 비활성화');
  }

  /// Update widget data on the native side
  static Future<void> updateWidget(
      {required String widgetType, required Map<String, dynamic> data}) async {
    try {
  ...
  static Future<bool> requestNotificationPermission() async {
    try {
      ...
      Logger.warning(
          '[NativePlatformService] 노티피케이션 권한 요청 실패 (선택적 기능, 수동 설정 필요): $e');
      return false;
    }
  }
  ...
  static Future<void> scheduleFortuneNotification(
    ...
    try {
      ...
      Logger.warning(
          '[NativePlatformService] 노티피케이션 예약 실패 (선택적 기능, 수동 알림 설정 필요): $e');
    }
  }
  ...
  static Future<void> cancelNotification(String id) async {
    try {
      ...
      Logger.warning(
          '[NativePlatformService] 노티피케이션 취소 실패 (선택적 기능, 예약된 알림 유지): $e');
    }
  }
...
class Android {
  ...
  Future<void> updateHomeWidget(
      {required int widgetId, required Map<String, dynamic> data}) async {
    if (const String.fromEnvironment('PLATFORM') != 'android') return;

    try {
      await NativePlatformService.androidChannel.invokeMethod(
          'updateHomeWidget', {'widgetId': widgetId, 'data': data});
      Logger.info('Native platform initialized successfully');
    } on PlatformException catch (e) {
      Logger.warning(
          '[NativePlatformService] 홈 위젯 업데이트 실패 (선택적 기능, Android 전용): $e');
    }
  }
  ...
  Future<Map<String, dynamic>?> getMaterialYouColors() async {
    ...
    try {
      final colors = await NativePlatformService.androidChannel
          .invokeMethod<Map<String, dynamic>>('getMaterialYouColors');
      Logger.info('Material You colors retrieved');
      return colors;
    } on PlatformException catch (e) {
      Logger.warning(
          '[NativePlatformService] Material You 색상 가져오기 실패 (선택적 기능, Android 12+ 전용): $e');
      return null;
    }
  }
  ...
  static Future<void> createNotificationChannel(
    ...
    try {
      ...
      Logger.warning(
          '[NativePlatformService] 노티피케이션 채널 생성 실패 (선택적 기능, Android 전용): $e');
    }
  }
}
