...
// 추천 운세 프로바이더
final recommendedFortunesProvider =
    FutureProvider<List<RecommendedFortune>>((ref) async {
  ...
  if (userProfile != null) {
    ...
    // 띠 기반 추천
    if (userProfile.zodiacAnimal != null &&
  ...
  for (final fortune in recentFortunes) {
    ...
    if (path.contains('love') ||
        path.contains('marriage') ||
        path.contains('compatibility')) {
  ...
  if (recommendations.length < 3) {
    ...
    for (final fortune in popularFortunes) {
      if (!recommendations.any((r) => r.id == fortune.id) &&
