...
final selectedTarotDeckProvider =
    StateNotifierProvider<SelectedTarotDeckNotifier, String>((ref) {
...
class SelectedTarotDeckNotifier extends StateNotifier<String> {
  ...
  Future<void> _loadSelectedDeck() async {
    ...
    if (savedDeckId != null &&
        TarotDeckMetadata.availableDecks.containsKey(savedDeckId)) {
      state = savedDeckId;
    }
  }
...
final tarotExperienceLevelProvider =
    StateNotifierProvider<TarotExperienceLevelNotifier, TarotDifficulty>((ref) {
...
class TarotExperienceLevelNotifier extends StateNotifier<TarotDifficulty> {
  ...
  Future<void> _loadExperienceLevel() async {
    ...
    if (savedLevel != null) {
      try {
        state = TarotDifficulty.values
            .firstWhere((level) => level.toString() == savedLevel);
      } catch (_) {
...
final preferredTarotStyleProvider =
    StateNotifierProvider<PreferredTarotStyleNotifier, TarotStyle?>((ref) {
...
class PreferredTarotStyleNotifier extends StateNotifier<TarotStyle?> {
  ...
  Future<void> _loadPreferredStyle() async {
    ...
    if (savedStyle != null) {
      try {
        state = TarotStyle.values
            .firstWhere((style) => style.toString() == savedStyle);
      } catch (_) {
...
final tarotDeckStatsProvider =
    StateNotifierProvider<TarotDeckStatsNotifier, Map<String, int>>((ref) {
...
class TarotDeckStatsNotifier extends StateNotifier<Map<String, int>> {
    ...
    if (statsJson != null) {
      try {
        final Map<String, dynamic> decoded =
            Map<String, dynamic>.from(Uri.splitQueryString(statsJson));
        state = decoded
  ...
  Future<void> incrementUsage(String deckId) async {
    state = {...state, deckId: (state[deckId] ?? 0) + 1};
  ...
  String? getMostUsedDeck() {
    ...
    return state.entries.reduce((a, b) => a.value > b.value ? a : b).key;
  }
}
