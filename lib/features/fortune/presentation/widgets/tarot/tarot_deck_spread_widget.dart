...
class _TarotDeckSpreadWidgetState extends State<TarotDeckSpreadWidget>
  ...
  void initState() {
    ...
    final fanController = _animationManager.createController(
        'fan', TarotAnimations.fanSpreadDuration);

    // Create staggered fan animations
    _fanAnimations = TarotAnimations.createFanAnimations(
        controller: fanController, cardCount: widget.cardCount);
  ...
  void _handleCardTap(int index) {
    debugPrint(
        '[TarotDeckSpread] Card tapped: $index, enableSelection: ${widget.enableSelection}');
    if (widget.enableSelection) {
      HapticUtils.lightImpact();
      widget.onCardSelected(index);
      debugPrint(
          '[TarotDeckSpread] Card selected event fired for index: $index');
    }
  }
          ...
          child: AnimatedContainer(
            ...
            child: Transform(
              ...
              transform: Matrix4.identity()
                ...
                ..rotateZ(isSelected
                    ? 0
                    : position.rotation * fanProgress) // 선택된 카드는 회전 해제
  ...
  Widget _buildGridSpread() {
    final crossAxisCount =
        (MediaQuery.of(context).size.width / (widget.cardWidth + 16)).floor();
              ...
              return AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  return Transform(
                    alignment: Alignment.center,
                    transform: Matrix4.identity()
                      ..translateByDouble(offset * progress, offset * progress,
                          index.toDouble(), 0.0)
                      ..scaleByDouble(
                          1.0 - (index * 0.02), 1.0 - (index * 0.02), 1.0, 1.0),
                    child: Opacity(
