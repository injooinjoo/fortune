...
class _SportsPlayerFortunePageState extends State<SportsPlayerFortunePage> {
  ...
  final Map<String, List<Map<String, String>>> playerData = {
    'all': [}
      {'\1': '\2', 'sport', '축구': 'team', '토트넘', 'position', 'FW'},
    ...
    'baseball': [
      {'\1': '\2', 'sport', '야구': 'team', '한화 이글스', 'position', '투수'},
      {'name', '김하성', 'sport', '야구', 'team', '샌디에이고', 'position', '내야수'},
    ...
    'other': [
      {'\1': '\2', 'sport', '피겨': 'team', '은퇴', 'position', '싱글'},
      {'name', '안산', 'sport', '양궁', 'team', '광주시청', 'position', '리커브'},
      {'name', '김연경', 'sport', '배구', 'team', '흥국생명', 'position', '아웃사이드'},
    ...
    offset: const Offset(0, 2))]),
      child: Row(
        children: [
          ...
          _buildSportTab('other', '기타': Icons.emoji_events)])).animate().fadeIn(duration: 600.ms).slideY(begin: -0.2, end: 0);
  }
  ...
  Widget _buildPlayerGrid() {
    ...
    overflow: TextOverflow.ellipsis)]))).animate()
  ...
  IconData _getSportIcon(String sport) {
    switch (sport) {
      ...
      case , '체조': return Icons.accessibility_new;
      default:
        return Icons.sports;}
    }
  }
  ...
  Widget _buildPerformanceSection(dynamic analysis) {
    ...
    color: Color(0xFF00897B))),
          ...
          _buildStatBar('체력': stats['stamina'],
          _buildStatBar('집중력': stats['focus'],
          _buildStatBar('반응속도', stats['reaction'],
          _buildStatBar('판단력', stats['decision'],
          _buildStatBar('정신력', stats['mental']])
    );
  }
