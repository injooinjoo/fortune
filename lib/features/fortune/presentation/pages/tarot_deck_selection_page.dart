...
class TarotDeckSelectionPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TarotDeckSelectionPage> createState() =>
      _TarotDeckSelectionPageState();
}
...
class _TarotDeckSelectionPageState extends ConsumerState<TarotDeckSelectionPage>
  ...
  void _confirmSelection() async {
    ...
    await ref
        ...
        .selectDeck(_tempSelectedDeckId!);

    // 사용 통계 업데이트
    await ref
        .read(tarotDeckStatsProvider.notifier)
        .incrementUsage(_tempSelectedDeckId!);
  ...
  Widget build(BuildContext context) {
    ...
    final mostUsedDeckId =
        ref.read(tarotDeckStatsProvider.notifier).getMostUsedDeck();
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (recommendedDecks.isNotEmpty) ...[
                              ...
                              const SizedBox(height: 16),
                              _buildDeckGrid(recommendedDecks, currentDeckId,
                                  mostUsedDeckId, fontScale, colors),
                              const SizedBox(height: 32),
                            ],
  ...
  Widget _buildSectionTitle(
      String title, double fontScale, DSColorScheme colors) {
    return Row(
      children: [
        Container(
          width: 4,
  ...
  Widget _buildExperienceLevelSection(
      ThemeData theme, double fontScale, DSColorScheme colors) {
            ...
            children: TarotDifficulty.values.map((level) {
              ...
              return ChoiceChip(
                ...
                onSelected: (selected) {
                  if (selected) {
                    ref
                        .read(tarotExperienceLevelProvider.notifier)
  ...
  Widget _buildDeckGrid(List<TarotDeck> decks, String currentDeckId,
      String? mostUsedDeckId, double fontScale, DSColorScheme colors) {
              ...
              GlassContainer(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        _buildTag(deck.difficulty.displayName,
                            deck.difficulty.color, fontScale, colors),
                        const SizedBox(width: 8),
                        _buildTag(deck.style.displayName, deck.primaryColor,
                            fontScale, colors),
                      ],
                    ),
                  ],
    ...
    return Stack(
      ...
      children: [
        if (deck.previewCards.length >= 3) ...[
          Transform.rotate(angle: -0.2, child: _buildPreviewCard(deck, 0)),
          Transform.rotate(angle: 0, child: _buildPreviewCard(deck, 1)),
          Transform.rotate(angle: 0.2, child: _buildPreviewCard(deck, 2)),
        ] else ...[
  ...
  Widget _buildTag(
      String text, Color color, double fontScale, DSColorScheme colors) {
    return Container(
