...
class TraditionalFortunePage extends BaseFortunePage {
  ...
  const TraditionalFortunePage({
    Key? key,
    this.initialType = TraditionalType.saju}) : super(
          key: key,
          title: '전통 운세',
...
class _TraditionalFortunePageState extends BaseFortunePageState<TraditionalFortunePage> {
  ...
  bool _requiresBirthTime() {
    return [
      ...
      TraditionalType.traditionalSaju].contains(_selectedType);
  }
  ...
  Widget buildCustomFortuneResult(Fortune fortune) {
    return Container(
      margin: const EdgeInsets.all(16),
      child: Column(
        children: [
          // Traditional content header
          if (_requiresBirthTime()) ...[
            Container(
              padding: const EdgeInsets.all(16),
              child: Text(
                '${_selectedType.label} 분석 결과',
