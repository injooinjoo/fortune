...
class _TimeBasedFortunePageState extends BaseFortunePageState<TimeBasedFortunePage> {
  ...
  void initState() {
    super.initState();
    final periodParam =
        widget.initialParams?['fortuneParams']?['period'] as String?;
    if (periodParam != null) {
  ...
  Future<void> _onGenerateFortune() async {
    ...
    try {
      ...
      Logger.error('❌ [TimeBasedFortunePage] Error in _onGenerateFortune', e);
      if (mounted) {
        setState(() {
          _isGeneratingFortune = false;
        });
  ...
  bool _isBirthday() {
    ...
    return userBirthDate.month == today.month &&
        userBirthDate.day == today.day;
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.pets_rounded, color: Colors.orange, size: 24),
              const SizedBox(width: 8),
              Text(
  ...
  String _getZodiacFortuneMessage(String zodiac) {
    final messages = {
      ...
      '전갈자리': '직관력이 뛰어한 날입니다. 내면의 목소리에 귀 기울이세요.',
  ...
  String _getChineseZodiacFortuneMessage(String zodiac) {
    final periodSpecific = _selectedPeriod == TimePeriod.today
        ? '오늘'
        : _selectedPeriod == TimePeriod.tomorrow
            ? '내일'
