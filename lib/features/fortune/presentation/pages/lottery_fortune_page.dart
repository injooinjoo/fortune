...
class _LotteryFortunePageState extends BaseFortunePageState<LotteryFortunePage> {
  ...
  Future<Fortune> generateFortune(Map<String, dynamic> params) async {
    ...
    // Generate a mock fortune for now
    final fortune = Fortune(
      id: 'lottery_${DateTime.now().millisecondsSinceEpoch}',
      userId: 'user_123',
      type: 'lucky-lottery',
      content: '오늘은 행운이 찾아올 좋은 날입니다.',
      createdAt: DateTime.now(),
      overallScore: 75,
      description: '당신의 숫자 감각이 빛을 발할 때입니다. 직감을 믿고 도전해보세요.',
      scoreBreakdown: {},
      luckyItems: {'color': '파란', 'number': 7},
      recommendations: ['행운의 번호를 믿어보세요', '적당한 금액으로 시작하세요'],
      category: '금전운'
    );
  ...
  void _extractLotteryData(Fortune fortune) {
    ...
    // Lucky tips
    _luckyTips = [
      '행운의 색상인 ${fortune.luckyItems?['color'] ?? '파란'}색 옷을 입고 구매하세요',
      '${fortune.luckyItems?['number'] ?? '7'}번이 들어간 번호를 선택해보세요',
