...
final batchFortuneProvider =
    StateNotifierProvider<BatchFortuneNotifier, BatchFortuneState>((ref) {
...
class BatchFortuneNotifier extends StateNotifier<BatchFortuneState> {
  ...
  BatchFortuneNotifier(this._service, this._authService, this._ref)
      : super(BatchFortuneState());
  ...
  Future<void> generateOnboardingFortunes() async {
    ...
    try {
      ...
      final results = await _service.generateOnboardingFortunes(
          userId: user.id, userProfile: userProfile);

      final savings =
          _service.calculateTokenSavings(BatchPackageType.onboarding);
  ...
  Future<void> refreshDailyFortunes() async {
    ...
    try {
      ...
      final savings =
          _service.calculateTokenSavings(BatchPackageType.dailyRefresh);
  ...
  Future<void> generatePackageFortunes(BatchPackageType packageType) async {
    ...
    try {
      ...
      final results = await _service.generateBatchFortunesByPackage(
          userId: user.id, packageType: packageType, userProfile: userProfile);
  ...
  Future<void> generateCustomBatchFortunes(List<String> fortuneTypes) async {
    ...
    try {
      ...
      state = state.copyWith(
          isLoading: false, results: results, currentPackage: null);
  ...
  BatchFortuneResult? getFortuneByType(String fortuneType) {
    return state.results?.firstWhere((result) => result.type == fortuneType,
...
final systemFortuneProvider =
    FutureProvider.family<Map<String, dynamic>, String>(
        (ref, fortuneType) async {
  final service = ref.watch(fortuneBatchServiceProvider);

  return await service.generateSystemFortunes(
      fortuneType: fortuneType, period: 'monthly', forceRegenerate: false);
});

/// 특정 MBTI 타입의 운세 가져오기
final mbtiFortuneProvider =
    Provider.family<Map<String, dynamic>?, String>((ref, mbtiType) {
...
/// 특정 혈액형의 운세 가져오기
final bloodTypeFortuneProvider =
    Provider.family<Map<String, dynamic>?, String>((ref, bloodType) {
...
/// 특정 별자리의 운세 가져오기
final zodiacFortuneProvider =
    Provider.family<Map<String, dynamic>?, String>((ref, zodiacSign) {
...
/// 특정 띠의 운세 가져오기
final zodiacAnimalFortuneProvider =
    Provider.family<Map<String, dynamic>?, String>((ref, zodiacAnimal) {
