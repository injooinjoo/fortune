...
class SajuNotifier extends StateNotifier<SajuState> {
  ...
  Future<void> calculateAndSaveSaju(
    ...
    try {
      ...
      final response =
          await _supabase.functions.invoke('calculate-saju', body: {
    ...
    return {
      'year': {
        ...
        'jiji': {
          'char': yearBranch,
          'hanja':
              rawData['year_branch_hanja'] ?? _getHanjaForBranch(yearBranch),
      ...
      'month': {
        ...
        'jiji': {
          'char': monthBranch,
          'hanja':
              rawData['month_branch_hanja'] ?? _getHanjaForBranch(monthBranch),
      ...
      'hour': hourStem != null
          ? {
              'cheongan': {
                'char': hourStem,
                'hanja':
                    rawData['hour_stem_hanja'] ?? _getHanjaForStem(hourStem),
                'element': _getElementForStem(hourStem)
              },
              'jiji': {
                'char': hourBranch,
                'hanja': rawData['hour_branch_hanja'] ??
                    _getHanjaForBranch(hourBranch),
  ...
  Future<void> refreshSaju() async {
    ...
    try {
      ...
      if (profile['birth_date'] == null) {
        state = state.copyWith(isLoading: false, error: '생년월일 정보가 없습니다.');
        return;
      }
