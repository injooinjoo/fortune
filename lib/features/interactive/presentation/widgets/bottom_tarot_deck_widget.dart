...
class BottomTarotDeckWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BottomTarotDeckWidget> createState() =>
      _BottomTarotDeckWidgetState();
}
...
class _BottomTarotDeckWidgetState extends ConsumerState<BottomTarotDeckWidget>
  ...
  void initState() {
    ...
    _slideUpAnimation = Tween<double>(begin: 1.0, end: 0.0).animate(
        CurvedAnimation(
            parent: _slideUpController, curve: Curves.easeOutCubic));
    ...
    _fanAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
        CurvedAnimation(parent: _fanController, curve: Curves.easeOutBack));
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
        animation: Listenable.merge(
            [_slideUpAnimation, _fanAnimation, _rotationAnimation]),
    ...
    return Positioned(
      ...
      child: Transform(
        ...
        transform: Matrix4.identity()
          ..translateByDouble(
              x * fanProgress, y * fanProgress, zIndex.toDouble(), 0.0)
  ...
  void _animateToCard(int index) {
    final targetRotation =
        -(index - cardCount / 2) / (cardCount / 2) * math.pi * 0.5;

    _rotationAnimation =
        Tween<double>(begin: _currentRotation, end: targetRotation).animate(
            CurvedAnimation(
                parent: _rotationController, curve: Curves.easeOut));
  ...
  void _animateToNearestCard(double velocity) {
    ...
    final nearestCardIndex = ((-_currentRotation / cardAngle) + cardCount / 2)
        .round()
        .clamp(0, cardCount - 1);
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            ...
            Positioned.fill(
              child: CustomPaint(
                painter: TarotCardBackPainter(isHighlighted: isCenter),
              ),
            ),
...
class TarotCardBackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw border pattern
    final borderRect = Rect.fromLTWH(size.width * 0.1, size.height * 0.05,
