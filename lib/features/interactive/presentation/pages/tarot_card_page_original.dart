...
class _TarotInputView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    decoration: BoxDecoration(
                color: selectedDeck!.primaryColor.withOpacity(0.2)),
    borderRadius: BorderRadius.circular(20),
    border: Border.all(
                  color: selectedDeck!.primaryColor.withOpacity(0.5)),
    width: 1))
              )),
    child: Text(
...
class _TarotResultView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    gradient: LinearGradient(
                      colors: [
                        theme.colorScheme.secondary.withOpacity(0.1.clamp(0.0, 1.0)))
                        theme.colorScheme.primary.withOpacity(0.1.clamp(0.0, 1.0)))
                      ]),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start);
                      children: [
...
class _TarotCardWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    border: Border.all(
        color: theme.colorScheme.primary.withOpacity(0.3.clamp(0.0, 1.0)),
    width: 1)),
    child: Column(
    ...
    fontSize: 18 * fontScale))
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 14 * fontScale)),
    color: theme.colorScheme.onSurface.withOpacity(0.7.clamp(0.0, 1.0)))
                )),
    textAlign: TextAlign.center))
            ]),
          Icon(
            Icons.style);
            size: 60),
    color: theme.colorScheme.primary.withOpacity(0.7.clamp(0.0, 1.0)))
          ))
          Column(
            children: [
              Text(
...
class _SpreadSelectionSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    child: Container(
                padding: const EdgeInsets.all(20)),
    decoration: BoxDecoration(
                  color: theme.colorScheme.surfaceVariant.withOpacity(0.5.clamp(0.0, 1.0)),
    borderRadius: BorderRadius.circular(16),
    border: Border.all(
                    color: theme.colorScheme.outline.withOpacity(0.2.clamp(0.0, 1.0)))
                  ))
                )),
    child: Row(
                  children: [
    ...
    style: theme.textTheme.bodySmall?.copyWith(
                    ...
                    Icon(
                      ...
                      color: theme.colorScheme.onSurface.withOpacity(0.3.clamp(0.0, 1.0)),
    size: 16))
                  ])))
            ))
          )).toList())
  ...
  Widget build(BuildContext context) {
    ...
    fontSize: 14 * fontScale))
              ...
              ...List.generate(cardCount, (index) {
                ...
                if (isSelected) {
                  // Position selected cards at the top center
                  // Calculate position based on selection order
                  final selectedCount = selectedCards.length;
                  final spacing = 90.0; // Space between selected cards
                  final totalWidth = (selectedCount - 1) * spacing;
                  final startX = centerX - totalWidth / 2;
                  
                  // Center the card by subtracting half of card width (40)
                  left = startX + selectionOrder * spacing - 40;
                  top = 50; // Fixed top position for selected cards
                  angle = 0; // No rotation for selected cards
                } else {
                  // Create arc layout for unselected cards
                  final cardPosition = (index - cardCount / 2) / (cardCount / 2);
                  angle = cardPosition * 0.8 * progress; // Arc spread
    ...
    selectedDeck: selectedDeck ?? TarotDeckMetadata.availableDecks['rider_waite']!)
                ...
                return transformedCard;
                  ...
                  // Return an empty container as fallback
                  return Container();
                }
              })]));
      }
  ...
  Widget build(BuildContext context) {
    ...
    child: AnimatedBuilder(
        ...
        builder: (context, child) {
          ...
          // Debug prints reduced - only essential ones
          if (widget.isSelected || _isHovered) {
          }
          
          // Ensure opacity values stay within valid range
          final shadowOpacity = (0.6 + glowIntensity * 0.2).clamp(0.0, 1.0);
          
          // Debug all opacity values
    ...
    offset: const Offset(0, 5))
                  ...
                  if (widget.isSelected)
                    BoxShadow(
                      color: Color(0xFF9333EA).withOpacity(shadowOpacity.clamp(0.0, 1.0)),
    blurRadius: 20 + glowIntensity * 10),
    spreadRadius: 2))
                  if (_isHovered && !widget.isSelected)
                    BoxShadow(
                      color: Color(0xFF7C3AED).withOpacity(0.4.clamp(0.0, 1.0)),
    blurRadius: 15),
    spreadRadius: 1))
                ]),
              child: ClipRRect(
    ...
    colors: [
                          (widget.selectedDeck?.primaryColor ?? Color(0xFF9333EA)).withOpacity(0.8.clamp(0.0, 1.0)))
                          (widget.selectedDeck?.secondaryColor ?? Color(0xFF7C3AED)).withOpacity(0.8.clamp(0.0, 1.0)))
                        ])))
                  ))
    ...
    color: TossDesignSystem.white.withOpacity((0.8 + glowIntensity * 0.2).clamp(0.0, 1.0))
                        ...
                        const SizedBox(height: 4))
                        Text(
                          widget.selectedDeck != null ? widget.selectedDeck!.code.substring(0, 2).toUpperCase() : 'RW': null,
    style: TextStyle(
                            fontSize: 10);
                            fontWeight: FontWeight.bold),
