...
class MiscFortuneApi {
  ...
  Future<Fortune> getWealthFortune(
    ...
    try {
      ...
      final response = await _apiClient.post(ApiEndpoints.wealthFortune,
          data: financialData ?? {});
      ...
      Logger.endTimer('getWealthFortune - Total', stopwatch);
      Logger.error('❌ [FortuneApiService] Unexpected error in getWealthFortune',
          e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<Fortune> getInvestmentEnhancedFortune(
      {required String userId, Map<String, dynamic>? investmentData}) async {
    ...
    try {
      Logger.debug('🔍 [FortuneApiService] Making API call',
          {'endpoint': ApiEndpoints.investmentEnhanced, 'method': 'POST'});

      final apiStopwatch = Logger.startTimer('API Call - investment-enhanced');
      final response =
          await _apiClient.post(ApiEndpoints.investmentEnhanced, data: {
        'userId': userId,
        ...?investmentData,
      });
      ...
      Logger.info(
          '✅ [FortuneApiService] getInvestmentEnhancedFortune completed', {
        'fortuneId': fortune.id,
        'overallScore': fortune.overallScore,
        'totalTime': '${stopwatch.elapsedMilliseconds}ms'
      });
      ...
      Logger.endTimer('getInvestmentEnhancedFortune - Total', stopwatch);
      Logger.error(
          '❌ [FortuneApiService] DioException in getInvestmentEnhancedFortune',
          {
            'type': e.type.toString(),
            'message': e.message,
            'statusCode': e.response?.statusCode
      ...
      Logger.endTimer('getInvestmentEnhancedFortune - Total', stopwatch);
      Logger.error(
          '❌ [FortuneApiService] Unexpected error in getInvestmentEnhancedFortune',
          e,
          stackTrace);
      rethrow;
    }
  ...
  Future<Fortune> getMbtiFortune(
    ...
    if (userId.isEmpty) {
      Logger.error('❌ [FortuneApiService] Invalid userId for MBTI fortune',
          {'userId': userId, 'mbtiType': mbtiType});
    ...
    if (mbtiType.isEmpty || mbtiType.length != 4) {
      Logger.error('❌ [FortuneApiService] Invalid MBTI type format',
          {'userId': userId, 'mbtiType': mbtiType, 'length': mbtiType.length});
    ...
    try {
      ...
      final response =
          await _apiClient.post(ApiEndpoints.mbtiFortune, data: requestData);
      ...
      Logger.debug('🔍 [FortuneApiService] Processing response data', {
        ...
        'hasSuccessKey':
            responseData is Map ? responseData.containsKey('success') : false,
        'hasDataKey':
      ...
      Logger.endTimer('getMbtiFortune - Total', stopwatch);
      Logger.error(
          '❌ [FortuneApiService] Unexpected error in getMbtiFortune',
          {
            'error': e.toString(),
            'mbtiType': mbtiType,
            'userId': userId,
  ...
  Future<Fortune> getWishFortune(
      ...
      required Future<Fortune> Function(
          ...
          getFortune}) async {
    return getFortune(
        fortuneType: 'wish', userId: userId, params: {'wish': wish});
  }
