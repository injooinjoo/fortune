...
class DailyFortuneApi {
  ...
  Future<Fortune> getDailyFortune(
    ...
    Logger.info('🔍 [FortuneApiService] getDailyFortune called',
        {'userId': userId, 'date': date?.toIso8601String()});
    ...
    final userProfileResponse = await supabase
        .from('user_profiles')
        .select(
            'name, birth_date, birth_time, gender, mbti, blood_type, zodiac_sign, chinese_zodiac')
        .eq('id', userId)
        .maybeSingle();
    ...
    if (cachedFortune != null) {
      Logger.info(
          '💾 [FortuneApiService] Cache hit! Returning cached daily fortune',
    ...
    try {
      final queryParams = {if (date != null) 'date': date.toIso8601String()};
      ...
      Logger.info('🔍 [FortuneApiService] Raw response data structure', {
        'responseKeys':
            responseData is Map ? responseData.keys.toList() : 'Not a map',
        'responseType': responseData.runtimeType.toString(),
        'hasFortuneKey':
            responseData is Map ? responseData.containsKey('fortune') : false,
        'hasDataKey':
            responseData is Map ? responseData.containsKey('data') : false,
        'hasStorySegmentsKey': responseData is Map
            ? responseData.containsKey('storySegments')
            : false,
      });
      ...
      final cacheSuccess = await _cacheService.cacheFortune(
          'daily', params, _entityToFortuneModel(fortune, 'daily'));
      ...
      if (_isNetworkError(e)) {
        ...
        final cachedFortune =
            await _cacheService.getCachedFortune('daily', params);
        if (cachedFortune != null) {
  ...
  Future<Fortune> getTodayFortune({required String userId}) async {
    Logger.debug(
        '📅 [FortuneApiService] getTodayFortune delegating to getDailyFortune',
        {'userId': userId, 'date': DateTime.now().toIso8601String()});
  ...
  Future<Fortune> getTomorrowFortune({required String userId}) async {
    ...
    Logger.debug(
        '📅 [FortuneApiService] getTomorrowFortune delegating to getDailyFortune',
        {'userId': userId, 'date': tomorrow.toIso8601String()});
  ...
  Future<Fortune> getWeeklyFortune(
      ...
      required Future<Fortune> Function(
          ...
          getFortune}) async {
    Logger.debug(
        '📅 [FortuneApiService] getWeeklyFortune delegating to getFortune',
        {'userId': userId, 'fortuneType': 'weekly'});
  ...
  Future<Fortune> getMonthlyFortune(
      ...
      required Future<Fortune> Function(
          ...
          getFortune}) async {
    Logger.debug(
        '📅 [FortuneApiService] getMonthlyFortune delegating to getFortune',
        {'userId': userId, 'fortuneType': 'monthly'});
  ...
  Future<Fortune> getYearlyFortune(
      ...
      required Future<Fortune> Function(
          ...
          getFortune}) async {
    Logger.debug(
        '📅 [FortuneApiService] getYearlyFortune delegating to getFortune',
        {'userId': userId, 'fortuneType': 'yearly'});
