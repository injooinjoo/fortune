...
class _HomeScreenState extends ConsumerState<HomeScreen> with AutomaticKeepAliveClientMixin {
  ...
  summary_card.DailyFortune? todaysFortune;
  fortune_entity.Fortune? cachedFortune; // 캐시된 전체 운세 데이터
  ...
  Future<void> _loadTodaysFortune() async {
    ...
    try {
      ...
      debugPrint('Error in _loadTodaysFortune: $e');
  ...
  void _updateFortuneUI(fortune_entity.Fortune fortune) {
    debugPrint('🔍 [HomeScreen] Updating UI with fortune data');
    
    // Log the actual API response for debugging
    debugPrint('metadata: ${fortune.metadata}');
    debugPrint('luckyItems: ${fortune.luckyItems}');
    debugPrint('overallScore: ${fortune.overallScore}');
    ...
    if (fortune.metadata != null && fortune.metadata!.containsKey('dailyFortune')) {
      ...
      setState(() {
        todaysFortune = summary_card.DailyFortune(
          score: score,
          ...
          caution: dailyData['caution'],
          bestTime: dailyData['bestTime'],
          compatibility: dailyData['compatibility'] ?? '좋은 사람들과 함께',
          elements: summary_card.FortuneElements(
  ...
  Future<void> _refreshFortune() async {
    ...
    try {
      ...
      debugPrint('Error refreshing fortune: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Padding(
                ...
                child: summary_card.DailyFortuneSummaryCard(
