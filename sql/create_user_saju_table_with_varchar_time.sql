...
-- birth_time 필드를 VARCHAR로 설정하여 한국 전통 시간 표기 저장
CREATE TABLE IF NOT EXISTS user_saju (
  ...
  birth_time VARCHAR(50),  -- 한국 전통 시간 표기 (예: "축시 (01:00 - 03:00)")
...
BEGIN
  ...
  SELECT 
    CASE 
      ...
      ELSE NULL
    END as saju_data,
    CASE WHEN us.id IS NOT NULL THEN true ELSE false END as has_data
...
-- 코멘트 추가
COMMENT ON TABLE user_saju IS '사용자별 사주팔자 정보';
COMMENT ON COLUMN user_saju.birth_time IS '생시 정보 (예: "축시 (01:00 - 03:00)")';
COMMENT ON COLUMN user_saju.element_balance IS '오행 분포 {"목": 2, "화": 1, "토": 2, "금": 2, "수": 1}';